...
1.  Link this extension to your local PLUMB installation:

    ```bash
    gemini extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your PLUMB session.
...
For security, PLUMB ignores any `allow` decisions or `yolo` mode configurations
contributed by extensions. This ensures that extensions can strengthen security
but cannot bypass user confirmation.
