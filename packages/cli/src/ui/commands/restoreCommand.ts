...
async function restoreAction(
  ...
  if (!checkpointDir) {
    return {
      type: 'message',
      messageType: 'error',
      content: 'Could not determine the .plumb directory path.',
    };
  }
  ...
  try {
    ...
    // 2. We trust that files valid according to this schema (written by usePlumbStream) contain the full HistoryItem structure.
