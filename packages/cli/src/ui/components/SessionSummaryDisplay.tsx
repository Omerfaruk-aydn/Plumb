...
export const SessionSummaryDisplay: React.FC<SessionSummaryDisplayProps> = ({
  ...
  const footerSessionId =
    ...
    !escapedSessionId.startsWith("'")
      ? `"${escapedSessionId}"`
      : escapedSessionId;
  let footer = `To resume this session: plumb --resume ${footerSessionId}`;

  if (worktreeSettings) {
    footer =
      `To resume work in this worktree: cd ${escapeShellArg(worktreeSettings.path, shell)} && plumb --resume ${footerSessionId}\n` +
