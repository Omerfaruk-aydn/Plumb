...
export async function main() {
  ...
  if (
    ...
    if (argv.runtimeIdentity) {
      ...
      exitCode = await printModelLimitsDiagnostics({
        provider: argv.diagnoseModelLimitsProvider ?? argv.provider,
        model: argv.model,
        summary: argv.diagnoseModelLimitsSummary,
      });
    } else if (argv.diagnoseTools) {
    ...
    } else if (argv.diagnoseToolRoute) {
      const { diagnoseToolRoute } = await import('./toolRouteProbe.js');
      exitCode = await diagnoseToolRoute(argv.provider, argv.model);
  ...
  // to run PLUMB. It is now safe to perform expensive initialization that
