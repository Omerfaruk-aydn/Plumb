...
describe('plumb.tsx main function', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('PLUMB_TRUST_WORKSPACE', 'true');
...
describe('initializeOutputListenersAndFlush', () => {
  ...
  it('should flush backlogs and setup listeners if no listeners exist', async () => {
    const { coreEvents } = await import('@plumb/core');
    const { initializeOutputListenersAndFlush } = await import('./plumb.js');
...
describe('plumb.tsx main function kitty protocol', { timeout: 30000 }, () => {
  ...
  it('should call setRawMode and detectCapabilities when isInteractive is true', async () => {
    ...
    vi.mocked(parseArguments).mockResolvedValue({
      ...
      diagnoseAuth: undefined,
      diagnoseModels: undefined,
      diagnoseAuthState: undefined,
      diagnosePlan: undefined,
      codingPlanStatus: undefined,
...
describe('plumb.tsx main function exit codes', () => {
  let originalEnvNoRelaunch: string | undefined;
  let originalIsTTY: boolean | undefined;
...
describe('startInteractiveUI', () => {
  ...
  it('should enable mouse events when alternate buffer is enabled', async () => {
    const { enableMouseEvents } = await import('@plumb/core');
    await startTestInteractiveUI(
      mockConfig,
      mockSettings,
      mockStartupWarnings,
  ...
  it('should perform all startup tasks in correct order', async () => {
    const { getVersion } = await import('@plumb/core');
  ...
  it('should not recordSlowRender when less than threshold', async () => {
    const { recordSlowRender } = await import('@plumb/core');
