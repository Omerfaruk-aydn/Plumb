...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find PLUMB documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available PLUMB documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to PLUMB's internal documentation.
