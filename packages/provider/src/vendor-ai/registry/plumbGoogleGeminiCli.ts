...
export const googleGeminiCliProvider = {
	...
	name: "Google Cloud Code Assist (PLUMB)",
	login: async (cb: OAuthLoginCallbacks) => {
		// Lazy import: keep heavy OAuth flow modules out of the eager registry graph.
		const { loginGeminiCli } = await import("./oauth/plumbGoogleGeminiCli.js");
	...
	refreshToken: async (credentials: OAuthCredentials) => {
		...
		const { refreshGoogleCloudToken } = await import("./oauth/plumbGoogleGeminiCli.js");
