...
async function httpEmailLogin(
  ...
  if (!trimmedOtp)
    throw new AIError.OAuthError('OTP code is required', {
      kind: 'validation',
      provider: 'perplexity',
    });
  if (ctrl.signal?.aborted) throw new AIError.LoginCancelledError();
  ...
  if (!verifyResponse.ok) {
    const reason =
      verifyData.text ??
      verifyData.error_code ??
      verifyData.status ??
      'OTP verification failed';
    throw new AIError.OAuthError(
      `Perplexity OTP verification failed: ${reason}`,
      {
        kind: 'validation',
        provider: 'perplexity',
  ...
  if (!verifyData.token) {
    throw new AIError.OAuthError(
      'Perplexity OTP verification response missing token',
      {
        kind: 'validation',
        provider: 'perplexity',
      },
 ...
 */
export async function loginPerplexity(
  ctrl: OAuthController,
): Promise<OAuthCredentials> {
  if (!ctrl.onPrompt) {
    throw new AIError.OnPromptRequiredError('Perplexity');
