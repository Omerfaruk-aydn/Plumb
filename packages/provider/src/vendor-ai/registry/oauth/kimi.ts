...
import { $env, getAgentDir } from '../../../vendor-shims/pi-utils.js';
...
function resolveOAuthHost(): string {
  return (
    $env.KIMI_CODE_OAUTH_HOST || $env.KIMI_OAUTH_HOST || DEFAULT_OAUTH_HOST
  );
}
...
async function requestDeviceAuthorization(): Promise<{
  ...
  if (!response.ok) {
    ...
    throw new AIError.OAuthError(
      `Kimi device authorization failed: ${response.status} ${text}`,
      {
        kind: 'device-auth',
        provider: 'kimi',
  ...
  const payload = parseAuthJsonResponse(await response.text(), {
    provider: 'kimi',
    label: 'Kimi device authorization',
    kind: 'device-auth',
    status: response.status,
  ...
  if (!userCode || !deviceCode || !verificationUri) {
    throw new AIError.OAuthError(
      'Kimi device authorization response missing required fields',
      {
        kind: 'validation',
        provider: 'kimi',
      },
...
function parseTokenPayload(
  ...
  if (!payload.access_token || typeof payload.expires_in !== 'number') {
    throw new AIError.OAuthError(
      'Kimi token response missing required fields',
      {
        kind: 'validation',
        provider: 'kimi',
      },
