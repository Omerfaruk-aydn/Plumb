...
import {
  ...
  OPENCODE_HEADERS,
} from '../../../vendor-catalog/wire/github-copilot.js';
...
async function fetchJson(
  ...
  return parseAuthJsonResponse(body, {
    provider: 'github-copilot',
    label: 'GitHub Copilot auth exchange',
...
async function startDeviceFlow(
  ...
  if (!data || typeof data !== 'object') {
    throw new AIError.OAuthError('Invalid device code response', {
      kind: 'validation',
      provider: 'github-copilot',
    });
  }
...
async function pollForGitHubAccessToken(
  ...
  let intervalMs = Math.max(
    pollIntervalFloorMs,
    Math.floor(intervalSeconds * pollIntervalScaleMs),
  );
  ...
  while (Date.now() < deadline) {
    ...
    const waitMs = Math.min(
      Math.ceil(intervalMs * intervalMultiplier),
      remainingMs,
    );
    try {
    ...
    if (
      ...
      const {
        ...
        interval,
      } = raw as DeviceTokenErrorResponse;
      ...
      if (error === 'slow_down') {
        ...
        intervalMs =
          typeof interval === 'number' && interval > 0
            ...
            : Math.max(
                pollIntervalFloorMs,
                intervalMs + 5 * pollIntervalScaleMs,
              );
      ...
      throw new AIError.OAuthError(
        `Device flow failed: ${error}${descriptionSuffix}`,
        {
          kind: 'polling',
          provider: 'github-copilot',
        },
  ...
  throw new AIError.OAuthError('Device flow timed out', {
    kind: 'timeout',
    provider: 'github-copilot',
  });
}
...
async function discoverGitHubCopilotApiEndpoint(
  token: string,
  fetchImpl: FetchImpl,
  ...
  try {
    ...
    return typeof endpoints?.api === 'string'
      ? normalizeGitHubCopilotApiEndpoint(endpoints.api)
      : undefined;
  } catch {
    return undefined;
  }
...
async function enableAllGitHubCopilotModels(
  ...
  const wireModelIds = [
    ...new Set(
      getBundledModels('github-copilot').map(
...
export async function loginGitHubCopilot(
  ...
  const domain =
    normalizedDomain && isPublicGitHubHost(normalizedDomain)
      ? 'github.com'
  ...
  const apiEndpoint = await discoverGitHubCopilotApiEndpoint(
    githubAccessToken,
    fetchImpl,
  );
