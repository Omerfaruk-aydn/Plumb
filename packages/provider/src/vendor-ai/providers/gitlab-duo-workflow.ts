...
} from '../../vendor-catalog/discovery/gitlab-duo-workflow.js';
...
import { normalizeSystemPrompts } from '../utils.js';
import { AssistantMessageEventStream } from '../utils/event-stream.js';
import { toolWireSchema } from '../utils/schema/wire.js';
import { readFileSync } from 'node:fs';
import { join } from 'node:path';
const chatmlHistoryNote = readFileSync(
  join(import.meta.dirname ?? __dirname, 'gitlab-duo-workflow-chatml-note.md'),
  'utf-8',
);
import { redactSensitiveCredentials } from './transform-messages.js';
...
const GITLAB_DUO_WORKFLOW_ACTION_NAMES = [
  'runMCPTool',
  'run_mcp_tool',
] as const;
...
export interface GitLabDuoWorkflowProviderSessionState
...
export function buildGitLabDuoWorkflowCreateBody(
  ...
  return {
    workflow_definition:
      options.workflowDefinition ?? GITLAB_DUO_WORKFLOW_DEFINITION,
    environment: 'ide',
    allow_agent_to_request_user: false,
    agent_privileges: [6],
    pre_approved_agent_privileges: [6],
    requires_duo_cli_enabled: false,
    ...(namespaceId && !options.projectId
...
export function buildGitLabDuoWorkflowWebSocketUrl(
  ...
  if (options.projectId)
    wsUrl.searchParams.set('project_id', options.projectId);
  if (options.namespaceId && !options.serviceEndpoint)
    wsUrl.searchParams.set(
      'namespace_id',
      toGitLabRestNamespaceId(options.namespaceId),
    );
  if (options.rootNamespaceId)
    wsUrl.searchParams.set(
      'root_namespace_id',
      toGitLabRestNamespaceId(options.rootNamespaceId),
    );
  ...
  if (options.workflowDefinition)
    wsUrl.searchParams.set('workflow_definition', options.workflowDefinition);
...
export function buildGitLabDuoWorkflowWebSocketHeaders(options: {
  ...
  const base = new URL(
    normalizeGitLabBaseUrl(options.baseUrl ?? DEFAULT_GITLAB_BASE_URL),
  );
  return {
    ...
    'x-gitlab-language-server-version':
      GITLAB_DUO_WORKFLOW_LANGUAGE_SERVER_VERSION,
    ...
    ...(options.namespaceId
      ? {
          'x-gitlab-namespace-id': toGitLabRestNamespaceId(options.namespaceId),
        }
      : {}),
    ...(options.rootNamespaceId
      ? {
          'x-gitlab-root-namespace-id': toGitLabRestNamespaceId(
            options.rootNamespaceId,
          ),
        }
      : {}),
...
export function buildGitLabDuoWorkflowStartRequest(
  ...
  const workflowMetadata = buildGitLabDuoWorkflowStartMetadata(
    model,
    availableModels,
    metadataOptions,
  );
  ...
  return {
    ...
    clientVersion: '1.0',
    workflowDefinition:
      metadataOptions.workflowDefinition ?? GITLAB_DUO_WORKFLOW_DEFINITION,
    ...
    flowConfig: buildGitLabDuoWorkflowInlineFlowConfig(
      buildGitLabDuoWorkflowSystemPrompt(context),
    ),
  };
}
...
export function buildGitLabDuoWorkflowInlineFlowConfig(
  systemPrompt: string,
...
function buildGitLabDuoWorkflowStartMetadata(
  ...
  return {
    ...
    ...(metadataOptions.projectId
      ? { projectId: metadataOptions.projectId }
      : undefined),
    ...
    ...(metadataOptions.rootNamespaceId
      ? {
          rootNamespaceId: toGitLabRestNamespaceId(
            metadataOptions.rootNamespaceId,
          ),
        }
      : undefined),
    selectedModelIdentifier: selectGitLabDuoWorkflowModelRef(
      model.id,
      availableModels,
    ),
  };
...
export function buildGitLabDuoWorkflowMcpTools(
  tools: Tool[] | undefined,
): GitLabMcpToolDefinition[] {
...
export function buildGitLabPlainTextFromToolResult(
  toolResult: ToolResultMessage,
): GitLabPlainTextResponse {
...
function findGitLabDuoWorkflowToolResultById(
  ...
  for (let index = messages.length - 1; index >= 0; index--) {
    ...
    if (message?.role === 'toolResult' && message.toolCallId === requestID)
      return message;
  }
  return undefined;
}
...
function resolveGitLabDuoWorkflowActionBatch(
  ...
  for (const action of actions) {
    const result = findGitLabDuoWorkflowToolResultById(
      messages,
      action.requestID,
    );
...
function buildGitLabDuoWorkflowResponseFromToolResult(
  toolResult: ToolResultMessage,
): GitLabPlainTextResponse {
...
function detectGitLabDuoWorkflowStall(
  state: GitLabDuoWorkflowStreamState,
): boolean {
...
function buildGitLabDuoWorkflowActionToolCall(
  action: GitLabDuoWorkflowActionDescriptor,
): ToolCall {
  const args =
    action.args &&
...
function mapGitLabDuoWorkflowMcpToolCall(args: Record<string, unknown>): {
  ...
  const rawName =
    ...
    '';
  const toolName = rawName.startsWith('mcp__omp__')
    ? rawName.slice('mcp__omp__'.length)
    : rawName;
  const parsedArgs = parseGitLabDuoWorkflowMcpArguments(
...
function parseGitLabDuoWorkflowMcpArguments(
  value: unknown,
...
function getGitLabDuoWorkflowProviderSessionState(
  ...
  const key = gitLabDuoWorkflowProviderSessionStateKey(
    baseUrl,
    modelId,
    sessionId,
  );
  const existing = providerSessionState.get(key) as
    | GitLabDuoWorkflowProviderSessionState
    | undefined;
...
// (never the raw token).
const gitLabDuoWorkflowAccountState = new Map<
  string,
  GitLabDuoWorkflowAccountState
>();

function gitLabDuoWorkflowAccountKey(
  apiKey: string,
  baseUrl: string,
...
function getGitLabDuoWorkflowCachedNamespace(
  ...
  return getGitLabDuoWorkflowAccountState(apiKey, baseUrl, cwd)
    .namespaceSelection;
}
...
function setGitLabDuoWorkflowCachedNamespace(
  ...
  getGitLabDuoWorkflowAccountState(apiKey, baseUrl, cwd).namespaceSelection =
    selection;
}
...
function isGitLabDuoWorkflowSettingsEnsured(
  apiKey: string,
  baseUrl: string,
  cwd: string | undefined,
): boolean {
  return (
    getGitLabDuoWorkflowAccountState(apiKey, baseUrl, cwd).settingsEnsured ===
    true
  );
}

function markGitLabDuoWorkflowSettingsEnsured(
  apiKey: string,
  baseUrl: string,
  cwd: string | undefined,
): void {
...
function hasGitLabDuoWorkflowExplicitNamespace(
  options: GitLabDuoWorkflowOptions,
): boolean {
  return Boolean(
...
function gitLabDuoWorkflowRestSignal(callerSignal?: AbortSignal): AbortSignal {
  const timeoutSignal = AbortSignal.timeout(
    GITLAB_DUO_WORKFLOW_REST_TIMEOUT_MS,
  );
  return callerSignal
...
async function readGitLabDuoWorkflowResponseErrorMessage(
  response: Response,
): Promise<string | undefined> {
  try {
    const payload: unknown = await response.json();
    const message =
      getGitLabDuoWorkflowErrorField(payload, 'message') ??
      getGitLabDuoWorkflowErrorField(payload, 'error');
...
function getGitLabDuoWorkflowErrorField(
  payload: unknown,
  field: 'message' | 'error',
): string | undefined {
...
async function runGitLabDuoWorkflow(
  ...
  const baseUrl = normalizeGitLabBaseUrl(
    model.baseUrl || DEFAULT_GITLAB_BASE_URL,
  );
  ...
  const steeredMidBatch = Boolean(
    resolvedBatch &&
      hasGitLabDuoWorkflowSteerAfterBatch(context.messages, resolvedBatch),
  );
  if (pendingSession && resolvedBatch && !steeredMidBatch) {
    const responses = resolvedBatch.map(({ requestID, result }) =>
      buildGitLabDuoWorkflowActionResponse(
        requestID,
        buildGitLabDuoWorkflowResponseFromToolResult(result),
      ),
    );
  ...
  const abandonStaleSession = Boolean(
    pendingSession &&
      (steeredMidBatch ||
        (pendingActions && pendingActions.length > 0 && !resolvedBatch)),
  );
  if (abandonStaleSession && pendingSession) {
    traceGitLabDuoWorkflow(
      steeredMidBatch
        ? 'workflow.steer_restart'
        : 'workflow.stale_action_restart',
      {
        workflowId: pendingSession.workflowId,
      },
    );
    ...
    await stopGitLabDuoWorkflow(
      fetchImpl,
      baseUrl,
      apiKey,
      pendingSession.workflowId,
    );
  }
  const workflowDefinition = resolveGitLabDuoWorkflowDefinition(
  ...
  const setupForNamespace = async (
    ...
    const createNamespaceId =
      namespaceSelection.namespacePath ?? restNamespaceId;
    ...
    if (
      ...
      // flags are still off.
      if (
        await ensureGitLabDuoWorkflowSettings(
          fetchImpl,
          baseUrl,
          apiKey,
          restNamespaceId,
    ...
    const configuredProjectIdIsPath = Boolean(
      configuredProjectId?.includes('/'),
    );
    const numericConfiguredProjectId = configuredProjectIdIsPath
      ? undefined
      : configuredProjectId;
    const pathConfiguredProjectId = configuredProjectIdIsPath
      ? configuredProjectId
      : undefined;
    const projectPath =
      configuredProjectPath ??
      pathConfiguredProjectId ??
      discoveredProject?.path;
    const projectId = numericConfiguredProjectId ?? discoveredProject?.id;
    const restProjectId =
      configuredProjectPath ?? configuredProjectId ?? discoveredProject?.path;
    const webSocketProjectId =
      ...
      (projectPath
        ...
        : undefined);
    const workflowConnection: GitLabDuoWorkflowDirectAccessConnection =
    ...
    const selectedModelIdentifier = selectGitLabDuoWorkflowModelRef(
      model.id,
      availableModels,
    );
  ...
  if (cachedNamespace) {
    try {
      ...
      setGitLabDuoWorkflowCachedNamespace(
        apiKey,
        baseUrl,
        options.cwd,
        rediscovered,
  ...
  if (renderedGoalBytes >= GITLAB_DUO_WORKFLOW_GOAL_HARD_OVERFLOW_BYTES) {
    ...
    if (!state.stream.done) {
      ...
      state.output.errorMessage =
        buildGitLabDuoWorkflowGoalOverflowMessage(renderedGoalBytes);
      state.stream.push({
        type: 'error',
        reason: 'error',
        error: state.output,
  ...
  try {
    for (let attempt = 0; attempt < 12; attempt++) {
      const ws = openGitLabDuoWorkflowSocket(
        workflowConnection.baseUrl ?? baseUrl,
        {
      ...
      if (
        ...
        traceGitLabDuoWorkflow('websocket.step_limit_restart', {
          workflowId,
          restart: stepLimitRestarts,
        });
        await stopGitLabDuoWorkflow(fetchImpl, baseUrl, apiKey, workflowId);
        workflowId = await createGitLabDuoWorkflow(
          fetchImpl,
      ...
      if (
        ...
        traceGitLabDuoWorkflow('websocket.stall_restart', {
          workflowId,
          restart: stallRestarts,
        });
        await stopGitLabDuoWorkflow(fetchImpl, baseUrl, apiKey, workflowId);
        workflowId = await createGitLabDuoWorkflow(
          fetchImpl,
      ...
      if (
        ...
        traceGitLabDuoWorkflow('websocket.generic_error_retry', {
          workflowId,
      ...
      if (lastSocketResult === 'retryable_error' && !state.stream.done) {
        ...
        if (state.goalOverflowMessage)
          state.output.errorMessage = state.goalOverflowMessage;
        state.stream.push({
          type: 'error',
          reason: 'error',
          error: state.output,
      ...
      if (lastSocketResult === 'stalled' && !state.stream.done) {
        ...
        state.output.errorMessage =
          state.goalOverflowMessage ??
          state.output.errorMessage ??
          GITLAB_DUO_WORKFLOW_STALL_ERROR_MESSAGE;
        state.stream.push({
          type: 'error',
          reason: 'error',
          error: state.output,
    ...
    finalizeGitLabDuoWorkflowResumeResult(
      state,
      providerSessionState,
      lastSocketResult,
    );
  ...
  try {
    const response = await fetchImpl(gitLabApiUrl(baseUrl, '/api/graphql'), {
      ...
      body: JSON.stringify({
        ...
        variables: {
          rootNamespaceId: toGitLabGraphQLNamespaceId(rootNamespaceId),
        },
      }),
...
function parseGitLabAvailableModelsPayload(
  value: unknown,
): GitLabAvailableModelsPayload | undefined {
  ...
  return {
    ...
    selectableModels: parseGitLabAvailableModelArray(
      (value as Record<string, unknown>).selectableModels,
    ),
  };
}

function parseGitLabAvailableModel(
  value: unknown,
): GitLabAvailableModel | null {
  if (!value || typeof value !== 'object') return null;
  return {
    name: getRecordString(value, 'name') ?? null,
    ref: getRecordString(value, 'ref') ?? null,
  };
}

function parseGitLabAvailableModelArray(
  value: unknown,
): GitLabAvailableModel[] | undefined {
  if (!Array.isArray(value)) return undefined;
  return value
    .map(parseGitLabAvailableModel)
    .filter((model): model is GitLabAvailableModel => Boolean(model));
}
...
async function discoverGitLabDuoWorkflowProject(
  ...
  const query =
    'per_page=1&min_access_level=30&order_by=last_activity_at&sort=desc';
  const endpoints = [
...
async function requestGitLabDuoWorkflowDirectAccess(
  ...
  const response = await fetchImpl(
    gitLabApiUrl(baseUrl, '/api/v4/ai/duo_workflows/direct_access'),
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${apiKey}`,
  ...
  const serviceEndpoint =
    ...
    Boolean(payload.duo_workflow_service?.base_url);
  return {
    token,
    ...(serviceEndpoint && payload.duo_workflow_service?.base_url
      ? {
          baseUrl: normalizeGitLabDuoWorkflowServiceBaseUrl(
            payload.duo_workflow_service.base_url,
          ),
        }
      : {}),
    headers: serviceEndpoint
      ? (payload.duo_workflow_service?.headers ?? {})
      : {},
    serviceEndpoint,
  };
}
...
async function createGitLabDuoWorkflow(
  ...
  const response = await fetchImpl(
    gitLabApiUrl(baseUrl, '/api/v4/ai/duo_workflows/workflows'),
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${apiKey}`,
...
async function stopGitLabDuoWorkflow(
  ...
  try {
    await fetchImpl(
      gitLabApiUrl(
        baseUrl,
        `/api/v4/ai/duo_workflows/workflows/${encodeURIComponent(workflowId)}`,
      ),
      {
        method: 'PATCH',
        headers: {
...
async function ensureGitLabDuoWorkflowSettings(
  ...
  try {
    const response = await fetchImpl(
      gitLabApiUrl(
        baseUrl,
        `/api/v4/groups/${encodeURIComponent(restNamespaceId)}`,
      ),
      {
        method: 'PUT',
        headers: {
    ...
    traceGitLabDuoWorkflow('settings.ensure', {
      status: response.status,
      ok: response.ok,
    });
    return response.status < 500;
  } catch (error) {
    traceGitLabDuoWorkflow('settings.ensure_error', {
      error: gitLabDuoWorkflowErrorText(error),
    });
    return false;
  }
...
function openGitLabDuoWorkflowSocket(
  ...
  const factory =
    options.webSocketFactory ?? defaultGitLabDuoWorkflowWebSocketFactory;
...
export function runGitLabDuoWorkflowSocket(
  ...
  resumeResponse?:
    ...
    | readonly GitLabDuoWorkflowActionResponse[],
  replayMessages?: readonly unknown[],
): Promise<GitLabDuoWorkflowSocketResult> {
  const { promise, resolve, reject } =
    Promise.withResolvers<GitLabDuoWorkflowSocketResult>();
  let settled = false;
  ...
  const settle = (
    result: GitLabDuoWorkflowSocketResult = 'closed',
    error?: unknown,
  ): void => {
    if (settled) return;
    settled = true;
  ...
  const resetIdleTimer = (): void => {
    ...
    idleTimer = setTimeout(() => {
      traceGitLabDuoWorkflow('websocket.idle_timeout', {
  ...
  const abort = (): void => {
    ...
    settle(
      'closed',
      new AIError.AbortError('GitLab Duo Workflow request aborted'),
    );
  };
  ...
  ws.onerror = (event) => {
    ...
    settle(
      ...
      new AIError.ProviderResponseError(
        `GitLab Duo Workflow WebSocket error: ${detail}`,
        {
          provider: 'gitlab-duo-agent',
          kind: 'runtime',
        },
  ...
  ws.onclose = (event) => {
    traceGitLabDuoWorkflow('websocket.close', {
      code: event.code,
      reason: event.reason,
    });
  ...
  if (replayMessages && replayMessages.length > 0) {
    ...
    resumeResponse &&
    (!Array.isArray(resumeResponse) || resumeResponse.length > 0)
  ) {
    ws.onopen = null;
    ...
    const responses = Array.isArray(resumeResponse)
      ? resumeResponse
      : [resumeResponse];
...
async function handleGitLabDuoWorkflowSocketMessage(
  ...
  traceGitLabDuoWorkflow('websocket.message', {
    ...
    hasCheckpoint: Boolean(
      getRecord(event, 'newCheckpoint') ?? getRecord(event, 'checkpoint'),
    ),
  ...
  if (isGitLabWorkflowCompletionStatus(status)) {
    traceGitLabDuoWorkflow('websocket.terminal', {
      status,
      checkpointLength: checkpoint?.contentLength ?? 0,
    });
  ...
  if (status === 'FAILED' || status === 'STOPPED') {
    ...
    if (
      status === 'FAILED' &&
      isGitLabDuoWorkflowGenericProcessingError(message)
    ) {
...
function isGitLabDuoWorkflowGenericProcessingError(message: string): boolean {
  return message
    .toLowerCase()
    .includes('error processing your request in the duo agent platform');
}
...
function hydrateGitLabDuoWorkflowCheckpointState(
  ...
  state.checkpointAgentContentSignatures =
    session.checkpointAgentContentSignatures;
}

function syncGitLabDuoWorkflowCheckpointState(
  state: GitLabDuoWorkflowStreamState,
): void {
  const active = state.providerSessionState?.active;
  if (!active) return;
  active.checkpointAgentContentByKey = state.checkpointAgentContentByKey;
  active.checkpointAgentContentSignatures =
    state.checkpointAgentContentSignatures;
}
...
function emitGitLabDuoWorkflowText(
  state: GitLabDuoWorkflowStreamState,
  text: string,
): void {
  if (!text) return;
...
function emitGitLabDuoWorkflowThinking(
  state: GitLabDuoWorkflowStreamState,
  thinking: string,
): void {
  if (!thinking) return;
...
function endGitLabDuoWorkflowThinking(
  state: GitLabDuoWorkflowStreamState,
): void {
...
function buildGitLabDuoWorkflowSystemPrompt(context: Context): string {
  ...
  return base
    ? `${base}\n\n${GITLAB_DUO_WORKFLOW_CHATML_HISTORY_NOTE}`
    : GITLAB_DUO_WORKFLOW_CHATML_HISTORY_NOTE;
}
...
function buildGitLabDuoWorkflowGoal(context: Context): string {
  const conversation = buildGitLabDuoWorkflowConversationHistory(
    context.messages,
  );
  ...
  if (conversation.length <= 1) {
    return redactSensitiveCredentials(
      extractLatestUserPrompt(context.messages),
    );
  }
  return redactSensitiveCredentials(
    renderGitLabDuoWorkflowChatMl(conversation),
  );
}
...
function renderGitLabDuoWorkflowChatMl(
  conversation: readonly GitLabDuoWorkflowReplayMessage[],
): string {
  return conversation.map(renderGitLabDuoWorkflowChatMlTurn).join('\n');
}

function renderGitLabDuoWorkflowChatMlTurn(
...
function gitLabDuoWorkflowChatMlToolResultHeader(
  message: GitLabDuoWorkflowReplayMessage,
): string | undefined {
...
function renderGitLabDuoWorkflowChatMlToolCall(
  toolCall: GitLabDuoWorkflowReplayToolCall,
): string {
...
function gitLabDuoWorkflowAssistantToolCalls(
  message: AssistantMessage,
): GitLabDuoWorkflowReplayToolCall[] {
...
function stripGitLabDuoWorkflowReplayIntent(
  args: Record<string, unknown>,
): Record<string, unknown> {
...
function extractLatestUserPrompt(messages: readonly Message[]): string {
  ...
  if (index < 0) return '';
  return gitLabDuoWorkflowUserContentToText(
    messages[index] as Exclude<Message, AssistantMessage>,
  );
}

function findLatestGitLabDuoWorkflowUserMessageIndex(
  messages: readonly Message[],
): number {
  for (let index = messages.length - 1; index >= 0; index--) {
    const message = messages[index];
  ...
  if (message.role === 'assistant') {
    return message.content
      .map((item) => {
        ...
        if (item.type === 'thinking' || item.type === 'redactedThinking')
          return '';
        return '';
      })
      .join('\n');
...
function gitLabDuoWorkflowUserContentToText(
  message: Exclude<Message, AssistantMessage>,
): string {
...
export function describeGitLabDuoWorkflowSocketEvent(event: unknown): string {
  ...
  if (event && typeof event === 'object') {
    ...
    const error = socketEventErrorText(
...
export function traceGitLabDuoWorkflow(
  event: string,
  data: Record<string, unknown> = {},
): void {
  if (Bun.env[GITLAB_DUO_WORKFLOW_TRACE_ENV] !== '1') return;
  const traceFile =
    Bun.env[GITLAB_DUO_WORKFLOW_TRACE_FILE_ENV]?.trim() ||
    DEFAULT_GITLAB_DUO_WORKFLOW_TRACE_FILE;
...
function truncateGitLabTraceData(
  data: Record<string, unknown>,
): Record<string, unknown> {
...
function truncateGitLabTraceValue(value: unknown): unknown {
  ...
  if (Array.isArray(value))
    return value.slice(0, 20).map((item) => truncateGitLabTraceValue(item));
  if (value && typeof value === 'object')
    return truncateGitLabTraceData(value as Record<string, unknown>);
  return value;
}
...
function toGitLabGraphQLNamespaceId(rootNamespaceId: string): string {
  if (/^\d+$/.test(rootNamespaceId))
    return `gid://gitlab/Group/${rootNamespaceId}`;
  return rootNamespaceId;
}

function toGitLabRestNamespaceId(rootNamespaceId: string): string {
  const match = rootNamespaceId.match(
    /^gid:\/\/gitlab\/(?:Group|Namespace)\/(\d+)$/,
  );
  return match?.[1] ?? rootNamespaceId;
}

export function extractGitLabWorkflowToken(
  payload: GitLabDirectAccessResponse,
): string | undefined {
  return (
    payload.gitlab_rails?.token ??
...
function isGitLabDuoWorkflowInlineFlow(
  workflowDefinition: GitLabDuoWorkflowDefinition,
): boolean {
...
function parseGitLabDuoWorkflowSocketData(
  data: unknown,
): Record<string, unknown> | null {
  if (typeof data === 'string') return parseJsonRecord(data);
  if (data instanceof ArrayBuffer)
    return parseJsonRecord(new TextDecoder().decode(data));
...
function extractGitLabDuoWorkflowCheckpoint(
  ...
  const checkpoint =
    getRecord(action, 'newCheckpoint') ??
    getRecord(event, 'newCheckpoint') ??
    getRecord(event, 'checkpoint');
  ...
  const contextUsage = extractGitLabDuoWorkflowContextUsage(
    event,
    action,
    checkpoint,
  );
  ...
  const content = checkpointJson
    ? extractGitLabCheckpointEntries(checkpointJson)
    : undefined;
  if (content) {
...
const GITLAB_DUO_WORKFLOW_CONTEXT_AGENT_PRIORITY = [
  'Chat Agent',
  'context_builder',
];
...
function readGitLabDuoWorkflowAgentUsage(
  value: unknown,
): GitLabDuoWorkflowContextUsage | undefined {
  if (!value || typeof value !== 'object') return undefined;
...
function extractGitLabCheckpointEntries(
  ...
  for (let index = 0; index < chatLog.length; index++) {
    ...
    if (messageType === 'agent') {
      ...
      const isReasoning =
        getRecordString(record, 'message_sub_type') === 'reasoning';
...
function getGitLabDuoWorkflowLatestMessageType(
  chatLog: unknown[],
): string | undefined {
...
function extractGitLabDuoWorkflowAction(
  ...
  const wrappedAction =
    getRecord(event, 'action') ??
    getRecord(event, 'workflowAction') ??
    getRecord(event, 'toolCall');
  if (wrappedAction) {
    ...
    const requestID =
      ...
      getRecordString(event, 'requestId');
    const resolvedRequestID = requireGitLabDuoWorkflowRequestID(
      requestID,
      name,
      wrappedAction,
    );
    const args =
      getRecord(wrappedAction, 'args') ??
      getRecord(wrappedAction, 'arguments') ??
      wrappedAction;
    return {
      requestID: resolvedRequestID,
      name,
      args: withGitLabDuoWorkflowToolCallId(args, resolvedRequestID),
    };
  }
...
function withGitLabDuoWorkflowToolCallId(
  args: unknown,
  requestID: string,
): unknown {
  const record =
