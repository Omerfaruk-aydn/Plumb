 ...
 */
import { Database, type Statement } from '../vendor-shims/sqlite-database.js';
import { createHash } from 'node:crypto';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import { parseAlibabaTokenPlanCredential } from '../vendor-catalog/wire/alibaba-token-plan.js';
import { $env, getAgentDbPath, logger } from '../vendor-shims/pi-utils.js';
import type { ApiKeyResolver } from './auth-retry.js';
import * as AIError from './error.js';
import { isUsageLimitOutcome } from './error/rate-limit.js';
import {
...
import type {
  ...
  OAuthProvider,
  OAuthProviderId,
} from './registry/oauth/types.js';
import { getEnvApiKey, getEnvApiKeyName } from './stream.js';
...
import { resolveUsedFraction } from './usage.js';
import {
  alibabaTokenPlanRankingStrategy,
  alibabaTokenPlanUsageProvider,
} from './usage/alibaba-token-plan.js';
import { claudeRankingStrategy, claudeUsageProvider } from './usage/claude.js';
import { cursorUsageProvider } from './usage/cursor.js';
import { googleGeminiCliUsageProvider } from './usage/plumbGemini.js';
import { githubCopilotUsageProvider } from './usage/github-copilot.js';
import {
  antigravityRankingStrategy,
  antigravityUsageProvider,
} from './usage/plumbGoogleAntigravity.js';
import { kimiUsageProvider } from './usage/kimi.js';
import { minimaxCodeUsageProvider } from './usage/minimax-code.js';
import {
  ollamaCloudUsageProvider,
  ollamaUsageProvider,
} from './usage/ollama.js';
import {
  codexRankingStrategy,
  openaiCodexUsageProvider,
} from './usage/openai-codex.js';
import {
  type CodexResetConsumeCode,
  type CodexResetCredit,
  consumeCodexResetCredit,
  listCodexResetCredits,
} from './usage/openai-codex-reset.js';
import { opencodeGoUsageProvider } from './usage/opencode-go.js';
import { syntheticUsageProvider } from './usage/synthetic.js';
import { umansUsageProvider } from './usage/umans.js';
import { xaiOauthUsageProvider } from './usage/xai-oauth.js';
import { zaiRankingStrategy, zaiUsageProvider } from './usage/zai.js';
 ...
 * provider-specific apiKey shapes (e.g. GitHub Copilot / Google PLUMB
 ...
 * key usage on it (PLUMB / Antigravity).
 */
...
function resolveOpenAICodexPlanRequirement(
  ...
  if (
    bareModelId === 'gpt-5.6' ||
    GPT_56_PAID_CODEX_MODEL_PATTERN.test(bareModelId)
  )
    return 'paid';
  return 'none';
}
...
export class AuthStorage {
  ...
  async #selectApiKeyCredential(
    ...
    const blockScopes =
      strategy.blockScopes?.(rankingContext) ??
      (blockScope ? [blockScope] : []);
  ...
  #buildUsageReportCacheKey(request: UsageRequestDescriptor): string {
    ...
    const versionOverride =
      USAGE_REPORT_CACHE_KEY_VERSION_OVERRIDES[request.provider];
    const providerKey =
      versionOverride === undefined
        ? request.provider
        : `${versionOverride}:${request.provider}`;
  ...
  async #fetchUsageCached(
    ...
    const promise = (async () => {
      ...
      const retainLastGood =
        this.#usageProviderResolver?.(request.provider)
  ...
  recordUsageCost(
    ...
    const credential = this.#resolveObservedUsageCredential(
      provider,
      options?.sessionId,
    );
  ...
  #resolveObservedUsageCredential(
    provider: Provider,
    sessionId?: string,
  ...
  async getModelUsageHealth(
    ...
    const accounts = await Promise.all(
      pool.map(async ({ entry, index }): Promise<ModelUsageAccountHealth> => {
        ...
        const limits = this.#getScopedUsageLimits(
          strategy,
          report,
          rankingContext,
        );
  ...
  async redeemResetCredit(options: {
    ...
    if (!creditId) {
      ...
      const credit =
        list?.credits.find(
          (entry) => (entry.status ?? 'available') === 'available',
        ) ?? list?.credits[0];
      if (!credit)
        return {
          ok: false,
...
function normalizeStoredIdentityKey(
...
function serializeCredential(
  provider: string,
  credential: AuthCredential,
): SerializedCredentialRecord | null {
  if (credential.type === 'api_key') {
    const data =
...
function deserializeCredential(row: AuthRow): AuthCredential | null {
  ...
  if (row.credential_type === 'api_key') {
    ...
    if (typeof data.key === 'string') {
      ...
      return source
        ? { type: 'api_key', key: data.key, source }
...
function resolveProviderCredentialIdentityKey(
  provider: string,
  identifiers: string[],
): string | null {
  const emailIdentifier = identifiers.find((identifier) =>
  ...
  if (provider === 'anthropic' || provider === 'openai-codex') {
    ...
    const orgIdentifier = identifiers.find((identifier) =>
  ...
  const accountIdentifier = identifiers.find((identifier) =>
    identifier.startsWith('account:'),
  );
  if (accountIdentifier) return accountIdentifier;
  if (emailIdentifier) return emailIdentifier;
  const projectIdentifier = identifiers.find((identifier) =>
    identifier.startsWith('project:'),
  );
  if (projectIdentifier) return projectIdentifier;
  return null;
}

function resolveCredentialIdentityKey(
  provider: string,
  credential: AuthCredential,
): string | null {
  if (credential.type === 'api_key') return null;
  return resolveProviderCredentialIdentityKey(
    provider,
    extractOAuthCredentialIdentifiers(credential),
  );
}

function resolveRowCredentialIdentityKey(
  provider: string,
  row: AuthRow,
): string | null {
  ...
  return credential?.type === 'oauth'
    ? resolveCredentialIdentityKey(provider, credential)
    : null;
}
...
function matchesReplacementCredential(
  ...
  const incomingIdentityKey = resolveProviderCredentialIdentityKey(
    provider,
    incomingIdentifiers,
  );
  ...
  const orgIdentifier = incomingIdentifiers.find((identifier) =>
    identifier.startsWith('org:'),
  );
  if (orgIdentifier === undefined) return false;
  if (
    incomingIdentityKey !== orgIdentifier &&
    !incomingIdentityKey.endsWith(`|${orgIdentifier}`)
  )
    return false;
  if (existingIdentityKey === orgIdentifier) return true;
  const existingIdentifiers =
    existing.type === 'oauth' &&
    existingIdentityKey.endsWith(`|${orgIdentifier}`)
  ...
  for (const identifier of incomingIdentifiers) {
    const isBase =
      ...
      identifier.startsWith('project:');
    if (!isBase) continue;
    if (identifier.slice(identifier.indexOf(':') + 1) === orgQualifierId)
      continue;
...
function extractOAuthCredentialIdentifiers(
  credential: OAuthCredential,
): string[] {
...
function extractOAuthTokenIdentifiers(
  ...
  try {
    ...
    const authClaims =
      typeof openAiAuth === 'object' &&
      openAiAuth !== null &&
      !Array.isArray(openAiAuth)
...
export class SqliteAuthCredentialStore implements AuthCredentialStore {
  ...
  #inferAuthSchemaVersionFromColumns(cols: Array<{ name?: string }>): number {
    ...
    const hasIdentityKey = cols.some(
      (column) => column.name === 'identity_key',
    );
  ...
  getCredentialBlock(
    ...
    return typeof row?.blocked_until_ms === 'number'
  ...
  getCredentialBlockReconcileAfter(
    ...
    const persistedReconcileAfter =
      typeof row.updated_at === 'number'
        ? row.updated_at * 1000 + USAGE_REPORT_TTL_MS
        : 0;
  ...
  upsertCredentialBlock(block: StoredCredentialBlock): void {
    const isLegacyCodexBlock =
      block.providerKey === LEGACY_CODEX_BLOCK_PROVIDER_KEY &&
      block.blockScope === LEGACY_CODEX_BLOCK_SCOPE;
    const blockScopes = isLegacyCodexBlock
      ? CODEX_METER_BLOCK_SCOPES
      : [block.blockScope];
    const upsert = this.#db.transaction(() => {
    ...
    const reconcileAfterMs = Math.min(
      block.blockedUntilMs,
      Date.now() + USAGE_REPORT_TTL_MS,
    );
    for (const blockScope of blockScopes) {
  ...
  getClientUsageSummary(sinceMs: number): ClientUsageSummary {
    const clients = this.#db
      .query(
        'SELECT install_id, hostname, first_seen, last_seen FROM clients ORDER BY last_seen DESC',
      )
      .all() as Array<{
      install_id: string;
  ...
  #readAuthRevision(): number {
    const row = this.#db
      .query('SELECT revision FROM auth_change_revision WHERE id = 1')
      .get() as {
      revision?: number;
    } | null;
  ...
  #readLocalAuthRevision(): number {
    const row = this.#db
      .query('SELECT revision FROM auth_local_change_revision WHERE id = 1')
      .get() as {
      revision?: number;
    } | null;
    return row?.revision ?? 0;
