// @ts-nocheck
import { getGeminiCliHeaders } from "../providers/plumbGoogleGeminiCli.js";
import type {
	UsageAmount,
	UsageFetchContext,
	UsageFetchParams,
...
async function loadCodeAssist(
	...
	if (!response.ok) {
		...
		ctx.logger?.warn("PLUMB loadCodeAssist failed", {
...
async function fetchQuota(
	...
	if (!response.ok) {
		const errorText = await response.text();
		ctx.logger?.warn("PLUMB retrieveUserQuota failed", {
			status: response.status,
			error: errorText,
		});
		return undefined;
