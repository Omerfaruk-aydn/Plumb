import { fetchWithRetry } from "../../vendor-shims/pi-utils.js";
import { Effort } from "../effort.js";
import { isGlm52ReasoningEffortModelId } from "../identity/family.js";
import type { ModelManagerOptions } from "../model-manager.js";
import type { FetchImpl, ModelSpec, ThinkingConfig } from "../types.js";
import { discoveryFetch } from "../utils.js";
import { createBundledReferenceMap, createReferenceResolver } from "./bundled-references.js";
	...
	return {
		...
		fetchDynamicModels: async () => {
			...
			const response = await fetchWithRetry(`${baseUrl}/api/tags`, {
				...
				...(discoveryFetch(config?.fetch) ? { fetch: discoveryFetch(config?.fetch) } : {}),
