...
import { getModelDbPath } from '../vendor-shims/pi-utils.js';
...
export function writeModelCache<TApi extends Api = Api>(
  ...
  const stripped = models.map((m) => {
    ...
    const { headers: _, ...rest } = m as ModelSpec<TApi> & {
...
/**
 * Remove a single provider's cache entry (or the whole cache file when
 * `providerId` is omitted). PLUMB facade primitive: the legacy UI surface
 * exposes per-provider and global cache invalidation, which this cache
 * otherwise lacks.
 */
export function removeModelCacheEntry(
  providerId: string,
  dbPath?: string,
): void {
  const cache = loadCacheFile(dbPath);
...
/** Drop every provider entry from the model cache file. */
export function clearModelCache(dbPath?: string): void {
  saveCacheFile({ version: CACHE_SCHEMA_VERSION, entries: {} }, dbPath);
}
