// @ts-nocheck
import type { Effort } from '../../omp-catalog/effort.js';
import {
  toFirepassWireModelId,
  toFireworksWireModelId,
} from '../../omp-catalog/fireworks-model-id.js';
import {
  isGlm52ReasoningEffortModelId,
  isKimiK3ModelId,
} from '../../omp-catalog/identity.js';
import { getSupportedEfforts } from '../../omp-catalog/model-thinking.js';
import { calculateCost } from '../../omp-catalog/models.js';
import type {
  OpenAICompat,
...
import { parseAlibabaTokenPlanCredential } from '../../omp-catalog/wire/alibaba-token-plan.js';
import {
  COREWEAVE_PROJECT_HEADER,
  coreWeaveProjectHeaders,
  hasCoreWeaveProjectHeader,
  removeBlankCoreWeaveProjectHeaders,
} from '../../omp-catalog/wire/coreweave.js';
...
function normalizeSakanaRequestBaseUrl(
  baseUrl: string | undefined,
): string | undefined {
...
function resolveSakanaRequestBaseUrl(): string | undefined {
  return (
    normalizeSakanaRequestBaseUrl($env.SAKANA_BASE_URL) ??
    normalizeSakanaRequestBaseUrl($env.FUGU_BASE_URL)
  );
}
...
function applyCoreWeaveProjectHeader(headers: Record<string, string>): void {
  ...
  if (projectHeaders) {
    headers[COREWEAVE_PROJECT_HEADER] =
      projectHeaders[COREWEAVE_PROJECT_HEADER];
  }
}
...
export function resolveOpenAIRequestSetup(
  ...
  if (model.provider === 'github-copilot') {
    ...
    // silently produces `Authorization: Bearer ` (no token) below, which
    // GitHub rejects as "Authorization header is badly formatted" instead
    // of the actual problem (no stored Copilot credential).
    if (!rawApiKey) {
      throw new AIError.MissingApiKeyError(
        model.provider,
    ...
    baseUrl =
      resolveGitHubCopilotBaseUrl(model.baseUrl, rawApiKey) ?? model.baseUrl;
  }
  ...
  if (options.promptCacheSessionId && model.compat?.promptCacheSessionHeader) {
    setHeaderIfAbsent(
      headers,
      model.compat.promptCacheSessionHeader,
      options.promptCacheSessionId,
    );
  }

  if (options.defaultBaseUrl !== undefined) {
    baseUrl =
      baseUrl ?? ($env.OPENAI_BASE_URL?.trim() || options.defaultBaseUrl);
  }
...
export function applyOpenRouterReportedCost(
  model: Pick<Model, 'provider'>,
  usage: Usage,
  rawUsage: unknown,
): void {
...
export function calculateOpenAIUsageAccounting(
  accounting: OpenAIUsageAccountingInput,
): OpenAIUsageAccounting {
  const cacheWriteTokens =
    accounting.cacheWriteOpenRouter ?? accounting.cacheWriteDeepSeek ?? 0;
  ...
  const input = isDeepSeekUsage
    ...
    : Math.max(
        0,
        accounting.promptTokens - accounting.cachedTokens - cacheWriteTokens,
      );
  ...
  return {
    ...
    cacheWrite,
    totalTokens:
      input + accounting.outputTokens + accounting.cachedTokens + cacheWrite,
    ...(accounting.reasoningTokens > 0
...
export function normalizeOpenAIPromptCacheKey(
  sessionId: string | undefined,
): string | undefined {
  return normalizeOpenAIStableId(sessionId, 64, 'pc_');
}

export function normalizeOpenRouterResponsesSessionId(
  sessionId: string | undefined,
): string | undefined {
...
export function getOpenAIPromptCacheKey(
  ...
  if (resolveCacheRetention(options?.cacheRetention) === 'none')
    return undefined;
...
export function parseAzureDeploymentNameMap(
...
export function clearOpenAIStrictToolsState(
  state: OpenAIStrictToolsState,
): void {
...
export function isStrictToolsDisabledForScope(
  ...
  return (
    state?.strictTools.disabledModelScopes.has(
      `${scope.provider}:${scope.baseUrl ?? ''}:${scope.modelId}`,
    ) ?? false
  );
}
...
export function disableStrictToolsForScope(
  ...
  if (!scope) return;
  state?.strictTools.disabledModelScopes.add(
    `${scope.provider}:${scope.baseUrl ?? ''}:${scope.modelId}`,
  );
}

export function isOpenRouterAnthropicModel(
  model: OpenAIModelIdentity,
): boolean {
  return (
    model.provider === 'openrouter' &&
    model.id.toLowerCase().startsWith('anthropic/')
  );
}
...
export function resolveOpenAIOutputTokenParam(
  ...
  const requested =
    input.maxTokens ??
    (input.alwaysSendMaxTokens
      ? (input.modelMaxTokens ?? OPENAI_MAX_OUTPUT_TOKENS)
      : undefined);
  if (requested === undefined) return undefined;
  if (
    input.isOpenRouterHost &&
    !input.alwaysSendMaxTokens &&
    !input.maxTokensExplicit
  )
    return undefined;
...
export function applyOpenAIGatewayRouting(
  ...
  if (compat.isVercelGatewayHost && compat.vercelGatewayRouting) {
    ...
    if (routing.only || routing.order || (cacheEnabled && routing.caching)) {
      const gatewayOptions: Pick<
        VercelGatewayRouting,
        'only' | 'order' | 'caching'
      > = {};
      if (routing.only) gatewayOptions.only = routing.only;
      if (routing.order) gatewayOptions.order = routing.order;
      if (cacheEnabled && routing.caching)
        gatewayOptions.caching = routing.caching;
...
export function applyVercelResponsesCacheControls(
  ...
  params.caching = 'auto';
  if (routing.cacheAnchorItems !== undefined)
    params.cache_anchor_items = routing.cacheAnchorItems;
  // A configured 1h TTL is capped by resolved retention; default and short intentionally omit it.
  if (
    routing.cacheTtl !== undefined &&
    (routing.cacheTtl !== '1h' || cacheRetention === 'long')
  ) {
 ...
 */
export type OpenAICompletionsParams = Omit<
...
export type OpenAIReasoningDisableReason =
  | 'caller'
  | 'forced-tool-choice'
  | 'tool-choice'
  | 'not-requested';
...
export function mapOpenAIReasoningEffort(
  ...
  return (
    compat?.reasoningEffortMap?.[level] ??
    model.thinking?.effortMap?.[level] ??
    effort
  );
}

function isImplicitDisableWhenNotRequested(
  disableMode: OpenAIReasoningDisableMode,
): boolean {
  return (
...
export function resolveOpenAICompatPolicy<TApi extends Api>(
  ...
  const baseCompat = (options.compat ??
    model.compat) as OpenAICompatPolicyCompat;
  ...
  const requestedAndAllowed =
    requestedEffort !== undefined &&
    !options.disableReasoning &&
    modelSupported;
  const conflictDisableReason: OpenAIReasoningDisableReason | undefined =
    forcedToolChoiceSuppressesReasoning
      ? 'forced-tool-choice'
      : anyToolChoiceSuppressesReasoning
        ? 'tool-choice'
        : undefined;
  const disableReason: OpenAIReasoningDisableReason | undefined =
    options.disableReasoning ? 'caller' : conflictDisableReason;
  const enabledBeforeThinkingVariant =
    requestedAndAllowed && disableReason === undefined;
  ...
  const compat =
    enabled && baseCompat.whenThinking
      ? (baseCompat.whenThinking as OpenAICompatPolicyCompat)
      : baseCompat;
  const omitReasoningEffort =
    options.omitReasoningEffort ??
    (compat.omitReasoningEffort || !compat.supportsReasoningEffort);
  const disableMode = compat.reasoningDisableMode;
  let wireEffort =
    enabled && requestedEffort !== undefined
      ? mapOpenAIReasoningEffort(model, compat, requestedEffort)
      : undefined;
  ...
  return {
    ...
    reasoning: {
      ...
      disableReason:
        disableReason ??
        (disabledWithoutRequest || disabledByNoneEffort
          ? 'not-requested'
          : undefined),
      ...
      includeEncryptedReasoning:
        options.includeEncryptedReasoning ?? compat.includeEncryptedReasoning,
      filterReasoningHistory:
        options.filterReasoningHistory ?? compat.filterReasoningHistory,
    },
    tools: {
      strictResponsesPairing:
        options.strictResponsesPairing ??
        compat.strictResponsesPairing ??
        false,
    ...
    messages: {
      systemRole:
        modelSupported && compat.supportsDeveloperRole ? 'developer' : 'system',
      supportsDeveloperRole: compat.supportsDeveloperRole,
      supportsMultipleSystemMessages:
        compat.supportsMultipleSystemMessages ?? true,
    },
    stream: {
      stripSpecialTokens: compat.stripDeepseekSpecialTokens
        ? 'deepseek'
        : false,
...
export function applyChatCompletionsCompatPolicy(
  params: OpenAICompletionsParams,
  ...
  if (
    (!reasoning.modelSupported && !reasoning.disabled) ||
    !reasoning.supportsParams
  )
    return;
  if (reasoning.enabled) {
    switch (reasoning.disableMode) {
      case 'zai-thinking-disabled':
        ...
        if (
          reasoning.dialect === 'kimi' &&
          reasoning.wireEffort !== undefined
        ) {
          params.thinking = { type: 'enabled', effort: reasoning.wireEffort };
          if (policy.compat.thinkingKeep)
            params.thinking.keep = policy.compat.thinkingKeep;
          break;
        }
        ...
        if (policy.compat.thinkingKeep)
          params.thinking.keep = policy.compat.thinkingKeep;
        if (
          policy.compat.supportsReasoningEffort &&
          reasoning.wireEffort !== undefined
        ) {
      ...
      case 'openrouter-enabled-false':
        if (reasoning.wireEffort !== undefined) {
          (
            params as typeof params & { reasoning?: { effort?: string } }
          ).reasoning = {
...
export function applyChatCompletionsReasoningParams(
  ...
  options:
    | (ChatCompletionsReasoningOptions & { toolChoice?: unknown })
    | undefined,
): void {
 ...
 */
function isZaiReasoningEffortDialect(
  model: Model<'openai-completions'>,
  compat: ResolvedOpenAICompat,
): boolean {
  return (
...
export function resolveOpenAIResponsesOutputClamp(
  model: Pick<Model, 'provider' | 'maxTokens'>,
): number | undefined {
...
function normalizeOpenAIStableId(
  value: string | undefined,
  maxLength: number,
  hashPrefix: string,
): string | undefined {
...
export const OPENAI_RESPONSES_PROGRESS_EVENT_TYPES: ReadonlySet<string> =
  new Set([
    'response.created',
...
export function isOpenAIResponsesProgressEvent(event: unknown): boolean {
  ...
  return (
    typeof type === 'string' && OPENAI_RESPONSES_PROGRESS_EVENT_TYPES.has(type)
  );
}

export function encodeTextSignatureV1(
  id: string,
...
export function encodeResponsesToolCallId(
  callId: string,
...
function responsesToolCallKind(
  type: unknown,
): ResponsesToolCallKind | undefined {
...
function responsesToolOutputKind(
  type: unknown,
): ResponsesToolCallKind | undefined {
...
export function repairOrphanResponsesToolOutputs(
  ...
  for (let index = 0; index < input.length; index++) {
    ...
    if (
      !outputKind ||
      !callId ||
      precedingCalls.has(`${outputKind}\0${callId}`)
    ) {
      repaired?.push(item);
      continue;
    }
    ...
    if (text.length > ORPHAN_OUTPUT_LIMIT)
      text = `${text.slice(0, ORPHAN_OUTPUT_LIMIT)}\n...[truncated]`;
    repaired.push({
      type: 'message',
      role: 'assistant',
 ...
 */
export function repairOrphanResponsesToolCalls(
  input: ResponseInput,
): ResponseInput {
  ...
  for (let index = input.length - 1; index >= 0; index--) {
    ...
    const callKind = responsesToolCallKind(item.type);
    if (callKind && callId && !laterOutputs.has(`${callKind}\0${callId}`))
      orphanIndexes.add(index);
  }
  ...
  for (let index = 0; index < input.length; index++) {
    ...
    repaired.push({
      type:
        kind === 'custom' ? 'custom_tool_call_output' : 'function_call_output',
      call_id: callId,
...
function clampResponsesImageDetail(
  ...
  return resolved === 'original' && !supportsImageDetailOriginal
    ? 'auto'
    : resolved;
}
...
export function convertResponsesInputContent(
  ...
  const { textBlocks, imageBlocks, omittedImages } = partitionVisionContent(
    content,
    supportsImages,
  );
  ...
  for (const item of imageBlocks) {
    normalizedContent.push({
      type: 'input_image',
      detail: clampResponsesImageDetail(
        item.detail,
...
function resolveReplayCustomToolName(
  ...
  return (
    wireNameMap?.get(wireName) ??
    (wireName === 'apply_patch' ? 'edit' : wireName)
  );
}
...
function adaptResponsesReplayItemsForModel(
  ...
  for (const item of input) {
    ...
    if (
      !supportsComputerUse &&
      (item.type === 'computer_call' || item.type === 'computer_call_output')
    ) {
      changed = true;
...
export function buildResponsesInput<TApi extends Api>(
  options: BuildResponsesInputOptions<TApi>,
): ResponseInput {
  const messages: ResponseInput = [];
  const systemPrompts = options.systemRole
    ? normalizeSystemPrompts(options.context.systemPrompt)
    : [];
  for (const systemPrompt of systemPrompts) {
    messages.push({
      role: options.systemRole as 'system' | 'developer',
  ...
  const supportsCustomToolCalls =
    options.model.applyPatchToolType === 'freeform';
  ...
  const filterReasoning = <T extends { type?: string }>(items: T[]): T[] =>
    options.nativeHistory?.filterReasoning
      ? items.filter((item) => item?.type !== 'reasoning')
      : items;
  const includeThinkingSignatures =
    options.includeThinkingSignatures ?? options.nativeHistory?.replay ?? true;
  ...
  for (const msg of transformedMessages) {
    if (msg.role === 'user' || msg.role === 'developer') {
      const providerPayload = (
        msg as { providerPayload?: AssistantMessage['providerPayload'] }
      ).providerPayload;
      ...
      if (historyItems && shouldReplayPayloadItems) {
        const sanitizedItems = sanitizeOpenAIResponsesHistoryItemsForReplay(
        ...
        messages.push(
          ...(escapeControlTokens
            ? escapeReplayedClientText(replayItems)
            : replayItems),
        );
        knownCallIds = collectKnownCallIds(messages);
        for (const id of collectCustomCallIds(messages)) customCallIds.add(id);
        for (const id of collectComputerCallIds(messages))
          computerCallIds.add(id);
        msgIndex++;
        continue;
      }
      ...
      if (!content) continue;
      const developerText =
        options.developerStringContent &&
        msg.role === 'developer' &&
      ...
      if (historyItems) {
        const rawSanitizedHistoryItems =
          sanitizeOpenAIResponsesAssistantHistoryItemsForReplay(
            filterReasoning(historyItems),
            {
              supportsImageDetailOriginal,
        ...
        if (nativeReplayEnabled && sanitizedHistoryItems) {
          ...
          knownCallIds = collectKnownCallIds(messages);
          for (const id of collectCustomCallIds(messages))
            customCallIds.add(id);
          for (const id of collectComputerCallIds(messages))
            computerCallIds.add(id);
          msgIndex++;
          continue;
  ...
  const withRepairedOutputs = options.repairOrphanOutputs
    ? repairOrphanResponsesToolOutputs(messages)
    : messages;
...
type ResponsesReplayAssistantMessage = Omit<ResponseOutputMessage, 'id'> & {
  id?: string;
};

function parseResponseReasoningReplayItem(
  signature: string | undefined,
...
export function convertResponsesAssistantMessage<TApi extends Api>(
  ...
  const hasReplayableReasoningItem =
    ...
    assistantMsg.content.some(
      (block) =>
        block.type === 'thinking' &&
        parseResponseReasoningReplayItem(block.thinkingSignature) !== undefined,
    );
  ...
  for (const block of assistantMsg.content) {
    if (block.type === 'thinking' && assistantMsg.stopReason !== 'error') {
      ...
      const reasoningItem = parseResponseReasoningReplayItem(
    ...
    if (block.type === 'text') {
      ...
      if (!msgId) {
        if (hasReplayableReasoningItem) {
          ...
          msgId =
            unsignedTextBlocks === 0
              ? `msg_${msgIndex}`
              : `msg_${msgIndex}_${unsignedTextBlocks}`;
    ...
    if (block.providerMetadata?.type === 'computer') {
      ...
      outputItems.push({
        ...
        pending_safety_checks: structuredCloneJSON(
          block.providerMetadata.pendingSafetyChecks,
        ),
        status: 'completed',
      } as ResponseInput[number]);
      continue;
    ...
    if (block.customWireName && supportsCustomToolCalls) {
      const rawInput =
        typeof block.arguments?.input === 'string' ? block.arguments.input : '';
...
function insertResponsesToolOutput(
  messages: ResponseInput,
  output: ResponseInput[number],
): void {
  ...
  while (index > 0) {
    ...
    if (
      ...
      previous === null ||
      !syntheticToolImageMessages.has(previous)
    ) {
      break;
    }
    index -= 1;
...
export function appendResponsesToolResultMessages<TApi extends Api>(
  ...
  const output = isHarmonyDialectModel(model)
    ? escapeHarmonyControlTokens(rawOutput)
    : rawOutput;
  if (
  ...
  if (computerCallIds?.has(normalized.callId)) {
    ...
    insertResponsesToolOutput(messages, {
      ...
      acknowledged_safety_checks: structuredCloneJSON(
        toolResult.providerMetadata.acknowledgedSafetyChecks,
      ),
    } as ResponseInput[number]);
    return;
  }
...
function foldReasoningSummary(
  parts: ResponseReasoningItem['summary'] | undefined,
): string {
  if (!parts) return '';
  let canonical = '';
  for (const part of parts) {
    ...
    const extendsCanonical =
      text.startsWith(canonical) && text[canonical.length] === '\n';
    canonical =
      !canonical || extendsCanonical ? text : `${canonical}\n\n${text}`;
  }
  return canonical;
}
...
export function finalizeReasoningThinking(
  ...
  if (cutoff)
    return finalizeCutoffReasoningThinking(item, streamedThinking, cutoff);
  const summaryThinking =
...
function finalizeCutoffReasoningThinking(
  ...
  if (summaryThinking) {
    ...
    if (!cutoff.emitted || summaryThinking.startsWith(cutoff.emitted)) {
      const suffix = summaryThinking
        .slice(cutoff.emitted.length)
        .replace(/^\n+/, '');
  ...
  return item.content?.[0]?.type === 'reasoning_text'
    ? (item.content[0].text ?? '')
    : '';
}
...
export function finalizeMessageText(
  item: ResponseOutputMessage,
  streamedText: string,
): string {
 ...
 */
export function finalizeToolCallArgumentsDone(
  block: ResponsesToolCallBlock,
  args: string,
): void {
...
export function finalizeCustomToolCallInputDone(
  block: ResponsesToolCallBlock,
  input: string,
): void {
...
function getOpenAIResponsesTerminalEvent(
  event: ResponseStreamEvent,
): OpenAIResponsesTerminalStreamEvent | undefined {
  const type = (event as { type?: unknown }).type;
...
export function computerCallMetadata(
...
export async function processResponsesStream<TApi extends Api>(
  ...
  const prefixedFunctionCallItemKey = (
    callId: string | undefined,
  ): string | undefined => (callId ? `fc_${callId}` : undefined);
  ...
  const registerOpenItem = (
    ...
    if (typeof outputIndex === 'number')
      openItemsByOutputIndex.set(outputIndex, entry);
    if (itemId) openItemsByItemId.set(itemId, entry);
    if (alternateItemKey && alternateItemKey !== itemId)
      openItemsByItemId.set(alternateItemKey, entry);
    if (
  ...
  const lookupOpenFunctionCallItem = (event: {
    ...
    if (hasOpenItemKey(event))
      return lookupOpenToolCallAlias(event, 'function_call');
    const canContinuePreviousIdentifierlessDelta =
      typeof event.delta === 'string';
    if (
      canContinuePreviousIdentifierlessDelta &&
      identifierlessFunctionDeltaTarget
    ) {
      const targetIndex = openItemsInOrder.indexOf(
        identifierlessFunctionDeltaTarget,
      );
      const target =
        targetIndex >= 0 ? openItemsInOrder[targetIndex] : undefined;
      if (
        ...
        const shouldAdvanceFromTarget =
          shouldAdvanceIdentifierlessFunctionDelta(event, target) &&
          hasLaterUnfinishedFunctionCall(targetIndex);
    ...
    for (let index = 0; index < openItemsInOrder.length; index++) {
      ...
      if (
        ...
        if (
          ...
          hasLaterUnfinishedFunctionCall(index)
        ) {
          skippedStartedCandidate = true;
          continue;
        }
        if (canContinuePreviousIdentifierlessDelta)
          identifierlessFunctionDeltaTarget = candidate;
        return candidate;
      }
    }
    if (skippedStartedCandidate && startsJsonObjectDelta(event.delta))
      return undefined;
    return lastOpenItem?.item.type === 'function_call'
      ? lastOpenItem
      : undefined;
  };
  const closeOpenItem = (
    ...
    if (typeof outputIndex === 'number')
      openItemsByOutputIndex.delete(outputIndex);
    if (itemId) openItemsByItemId.delete(itemId);
    if (alternateItemKey && alternateItemKey !== itemId)
      openItemsByItemId.delete(alternateItemKey);
    if (
      prefixedAlternateItemKey &&
      prefixedAlternateItemKey !== itemId &&
      prefixedAlternateItemKey !== alternateItemKey &&
    ...
    if (entry && identifierlessFunctionDeltaTarget === entry)
      identifierlessFunctionDeltaTarget = undefined;
    if (entry && lastOpenItem === entry) lastOpenItem = null;
  };
  const contentIndexOf = (
    block: ThinkingContent | TextContent | StreamingToolCallBlock,
  ): number => output.content.indexOf(block);
  ...
  for await (const event of openaiStream) {
    ...
    if (event.type === 'response.created') {
      ...
      if (
        entry?.item.type === 'custom_tool_call' &&
        entry.block.type === 'toolCall'
      ) {
      ...
      if (item.type === 'reasoning') {
        ...
        const reasoningBlock =
          entry?.block.type === 'thinking'
            ...
            : (output.content.find(
                (b) =>
                  b.type === 'thinking' &&
                  (b as ThinkingContent).itemId === item.id,
              ) as ThinkingContent | undefined);
        if (reasoningBlock) {
          reasoningBlock.thinking = finalizeReasoningThinking(
            item,
        ...
        const block =
          entry?.block.type === 'toolCall' ? entry.block : undefined;
        ...
        const block =
          entry?.block.type === 'toolCall' ? entry.block : undefined;
        const rawInput = block?.[kStreamingPartialJson]
          ? block[kStreamingPartialJson]
          : (item.input ?? '');
        const toolCall: ToolCall = {
          type: 'toolCall',
        ...
        const image: ImageContent = {
          ...
          data: item.result,
          mimeType:
            parseImageMetadata(Buffer.from(item.result, 'base64'))?.mimeType ??
            'image/png',
        };
      ...
      if (response?.status === 'failed' || response?.status === 'cancelled') {
        const error =
          response?.error ?? (response as any)?.status_details?.error;
        ...
        const message = error
          ...
          : details?.reason
            ...
            : typeof statusDetailsReason === 'string' &&
                statusDetailsReason.length > 0
              ? `status_details: ${statusDetailsReason}`
              : 'Unknown error (no error details in response)';
        throw new AIError.ProviderResponseError(message, {
          provider: model.provider,
          kind: 'output',
        });
      }
      ...
      const error =
        event.response?.error ?? (event.response as any)?.status_details?.error;
...
export function mapOpenAIResponsesStopReason(
  ...
  switch (status) {
    ...
    default: {
      ...
      logger.warn('Unhandled OpenAI Responses stop reason', {
        status: exhaustive,
      });
      return 'stop';
    }
...
export function hasExecutableIncompleteResponsesToolCalls(
  output: AssistantMessage,
): boolean {
  ...
  for (const block of output.content) {
    ...
    if (pending.customWireName !== undefined || rawArguments === undefined)
      return false;
 ...
 */
export function finalizePendingResponsesToolCalls(
  output: AssistantMessage,
): void {
  for (const block of output.content) {
    if (block.type !== 'toolCall') continue;
...
export function promoteResponsesToolUseStopReason(
  ...
  if (
    ...
    (output.stopReason === 'stop' ||
      (promoteIncompleteToolUse && output.stopReason === 'length'))
  ) {
...
export function createInitialResponsesAssistantMessage(
  api: Api,
  provider: string,
  modelId: string,
): AssistantMessage {
...
type CommonResponsesParams = ResponseCreateParamsStreaming &
  ResponsesSamplingParamsExtras;
...
export function applyCommonResponsesSamplingParams<
  P extends CommonResponsesParams,
>(
  params: P,
  options: CommonSamplingOptions | undefined,
  model: Pick<
    Model,
    'provider' | 'api' | 'id' | 'omitMaxOutputTokens' | 'maxTokens'
  > & {
...
export function applyResponsesCompatPolicy<
  P extends ResponseCreateParamsStreaming,
>(
  params: P,
  policy: OpenAICompatPolicy,
  ...
  if (reasoning.disabled) {
    ...
    if (
      ...
      >;
      params.reasoning = {
        effort: reasoning.wireEffort as ReasoningParam['effort'],
      } as P['reasoning'] & ReasoningParam;
      return;
    }
    return;
  }
  ...
  if (
    ...
    const wireEffort =
      reasoning.wireEffort ?? options?.mapEffort?.(requested) ?? requested;
    type ReasoningParam = NonNullable<
      ResponseCreateParamsStreaming['reasoning']
    >;
...
export function populateResponsesUsageFromResponse(
  ...
  const reportedCachedTokens =
    details?.cached_tokens ?? usage.prompt_cache_hit_tokens ?? 0;
  const orchestrationInputTokens = details?.orchestration_input_tokens ?? 0;
  const orchestrationInputCachedTokens =
    details?.orchestration_input_cached_tokens ?? 0;
  const orchestrationOutputTokens =
    outputDetails?.orchestration_output_tokens ?? 0;
  ...
  const orchestrationInputCached = Math.min(
    orchestrationInputTokens,
    orchestrationInputCachedTokens,
  );
  const orchestrationInput = Math.max(
    0,
    orchestrationInputTokens - orchestrationInputCached,
  );
  const accounting = calculateOpenAIUsageAccounting({
    promptTokens: Math.max(
      0,
      reportedInputTokens -
        (primaryIncludesOrchestration ? orchestrationInputTokens : 0),
    ),
    outputTokens: Math.max(
      0,
      reportedOutputTokens -
        (primaryIncludesOrchestration ? orchestrationOutputTokens : 0),
    ),
    cachedTokens: Math.max(
      0,
      reportedCachedTokens -
        (primaryIncludesOrchestration ? orchestrationInputCached : 0),
    ),
  ...
  const orchestrationTotal =
    orchestrationInput + orchestrationInputCached + orchestrationOutputTokens;
  if (orchestrationTotal > 0) {
    accounting.orchestration = {
      ...(orchestrationInput > 0 ? { input: orchestrationInput } : {}),
      ...(orchestrationInputCached > 0
        ? { cacheRead: orchestrationInputCached }
        : {}),
      ...(orchestrationOutputTokens > 0
        ? { output: orchestrationOutputTokens }
        : {}),
    };
    accounting.totalTokens =
      reportedTotalTokens ?? accounting.totalTokens + orchestrationTotal;
  }
 ...
 */
export function buildResponsesDeltaInput<
  TItem extends ResponseInputItem | InputItem,
>(
  ...
  const baselineLen =
    (previous.input?.length ?? 0) + (previousResponseItems?.length ?? 0);
