...
} from '../../omp-catalog/compat/anthropic.js';
import {
  hostMatchesUrl,
  isVertexRawPredictUrl,
} from '../../omp-catalog/hosts.js';
import { mapEffortToAnthropicAdaptiveEffort } from '../../omp-catalog/model-thinking.js';
import { calculateCost, getBundledModel } from '../../omp-catalog/models.js';
import { isAnthropicOAuthToken } from '../../omp-catalog/utils.js';
import { parseGitHubCopilotApiKey } from '../../omp-catalog/wire/github-copilot.js';
import {
  $env,
  getInstallId,
  isEnoent,
  ...
  readSseEvents,
} from '../../omp-shims/pi-utils.js';
import { renderDemotedThinking } from '../dialect/demotion.js';
import * as AIError from '../error/index.js';
import { getEnvApiKey, OUTPUT_FALLBACK_BUFFER } from '../stream.js';
import type {
...
import { createAbortSourceTracker } from '../utils/abort.js';
import {
  clearStreamingPartialJson,
  kStreamingBlockIndex,
  kStreamingLastParseLen,
  kStreamingPartialJson,
} from '../utils/block-symbols.js';
import { withEmptyCompletionRetry } from '../utils/empty-completion-retry.js';
import { AssistantMessageEventStream } from '../utils/event-stream.js';
import { isFoundryEnabled } from '../utils/foundry.js';
import {
  finalizeErrorMessage,
  type RawHttpRequestDump,
} from '../utils/http-inspector.js';
import {
  getStreamFirstEventTimeoutMs,
  getStreamIdleTimeoutMs,
  iterateWithIdleTimeout,
} from '../utils/idle-iterator.js';
import { notifyProviderResponse } from '../utils/provider-response.js';
import {
  getHeadersFromError,
  getRetryAfterMsFromHeaders,
} from '../utils/retry-after.js';
import { COMBINATOR_KEYS, NO_STRICT, toolWireSchema } from '../utils/schema.js';
import { spillToDescription } from '../utils/schema/spill.js';
import { createSdkStreamRequestOptions } from '../utils/sdk-stream-timeout.js';
import { notifyRawSseEvent } from '../utils/sse-debug.js';
import { isForcedToolChoice } from '../utils/tool-choice.js';
import {
...
import {
  ...
  claudeToolPrefix,
  coworkUserAgent,
} from './claude-code-fingerprint.js';
import {
  buildCopilotDynamicHeaders,
  hasCopilotVisionInput,
  resolveGitHubCopilotBaseUrl,
} from './github-copilot-headers.js';
import { getOpenAIPromptCacheKey } from './openai-shared.js';
import { transformMessages } from './transform-messages.js';
import { NON_VISION_IMAGE_PLACEHOLDER } from './vision-guard.js';
...
function mergeAnthropicBetaHeader(
  ...
  for (const key in callerHeaders) {
    if (key.toLowerCase() === 'anthropic-beta') {
      return {
        [key]: buildBetaHeader(normalizeExtraBetas(callerHeaders[key]), [beta]),
      };
    }
  }
...
function buildCoworkBetas(
  ...
  for (const beta of agentRequest
    ...
    betas.push(beta);
    if (agentRequest && longContext && beta === 'claude-code-20250219')
      betas.push(context1mBeta);
  }
...
function isClaudeCodeClientUserAgent(
...
export function buildAnthropicHeaders(
  ...
  const incomingAuthorization = getHeaderCaseInsensitive(
    options.modelHeaders,
    'Authorization',
  );
  const incomingApiKey = getHeaderCaseInsensitive(
    options.modelHeaders,
    'X-Api-Key',
  );
  ...
  const betaHeader = buildBetaHeader(
    options.coworkBetas ??
      (oauthToken ? buildCoworkBetas(true, true, false) : []),
    extraBetas,
  );
  const acceptHeader = oauthToken
    ? 'application/json'
    : stream
      ? 'text/event-stream'
      : 'application/json';
    ...
    for (const key in headerSource) {
      ...
      if (enforcedHeaderKeys.has(lowerKey)) {
        if (
          ...
          overridableAnthropicHeaderKeys.has(lowerKey)
        ) {
  ...
  if (oauthToken) {
    ...
    const headers = {
      ...
      ...(options.claudeCodeSessionId
        ? { 'X-Claude-Code-Session-Id': options.claudeCodeSessionId }
        : {}),
      ...coworkHeaders,
      ...(betaHeader ? { 'anthropic-beta': betaHeader } : {}),
      'anthropic-dangerous-direct-browser-access': 'true',
    ...
    return allowAnthropicHeaderOverrides
      ? mergeHeaders(headers, anthropicHeaderOverrides)
      : headers;
 ...
 * forge the boundary.
 */
function anthropicProviderSessionStateKey(
  baseUrl: string,
  modelId: string,
): string {
...
function getAnthropicProviderSessionState(
  ...
  const existing = providerSessionState.get(key) as
...
export function clearAnthropicFastModeFallback(
  ...
  for (const [key, value] of providerSessionState) {
    if (key !== ANTHROPIC_PROVIDER_SESSION_STATE_KEY && !key.startsWith(prefix))
      continue;
...
export function isAnthropicFastModeFallbackDisabled(
  ...
  if (
    ...
    return false;
  const baseUrl =
    resolveAnthropicBaseUrl(model as Model<'anthropic-messages'>) ??
    'https://api.anthropic.com';
  const key = anthropicProviderSessionStateKey(baseUrl, model.id);
  return (
    (providerSessionState.get(key) as AnthropicProviderSessionState | undefined)
      ?.fastModeDisabled ?? false
  );
}

function hasStrictAnthropicTools(
...
function getCacheControl(
  ...
  const retention = isOAuthToken
    ...
    : resolveCacheRetention(
        cacheRetention,
        model.compat.supportsLongCacheRetention ? 'long' : 'short',
      );
  if (retention === 'none') {
    return { retention };
  }
  const ttl =
    retention === 'long' && model.compat.supportsLongCacheRetention
      ? '1h'
      : undefined;
  return {
    retention,
...
const overridableAnthropicHeaderKeys = new Set(
  [...Object.keys(coworkHeaders), 'anthropic-beta', 'User-Agent', 'x-app'].map(
    (key) => key.toLowerCase(),
  ),
);
...
function patchCch(
  body: Uint8Array,
): 'patched' | 'no-billing-header' | 'unanchored' {
  ...
  return (input, init) => {
    if (
      ...
      if (patchCch(encoded) === 'unanchored') {
        ...
        logger.warn(
          'anthropic: cch billing placeholder present but not patched; sending unattested request',
        );
      }
...
function readAnthropicMetadataAccountId(
  metadata: Record<string, unknown> | undefined,
): string | undefined {
  return (
...
function generateClaudeJsonUserId(
  sessionId?: string,
  accountId?: string,
): string {
...
export function resolveAnthropicMetadataUserId(
  ...
  if (typeof userId === 'string') {
    if (
      !isOAuthToken ||
      isClaudeCloakingUserId(userId) ||
      isClaudeJsonUserId(userId)
    ) {
      return userId;
    }
  }
...
function normalizeUmansWebSearchProvider(
  value: string | undefined,
): 'native' | 'exa' | undefined {
  const normalized = value?.trim().toLowerCase();
  return normalized === 'native' || normalized === 'exa'
    ? normalized
    : undefined;
}

function getUmansWebSearchProvider(
  headers: Record<string, string> | undefined,
): 'native' | 'exa' | undefined {
  const explicit = getHeaderCaseInsensitive(
    headers,
    UMANS_WEBSEARCH_PROVIDER_HEADER,
  );
...
function isUmansAnthropicModel(model: Model<'anthropic-messages'>): boolean {
  return (
    model.provider === 'umans' ||
    model.baseUrl.toLowerCase().includes('api.code.umans.ai')
  );
}
...
function shouldUseUmansGatewayWebSearch(
  name: string,
  enabled: boolean,
): boolean {
...
function encodeAnthropicToolName(
  ...
  if (shouldUseUmansGatewayWebSearch(name, useUmansGatewayWebSearch))
    return name;
...
function decodeAnthropicToolName(
  ...
  if (isOAuthToken || escapeBuiltinToolNames)
...
async function resizeAnthropicManyImageBlock(
  block: ImageContent,
  ...
  try {
    ...
    const scale = Math.min(
      ANTHROPIC_MANY_IMAGE_MAX_DIMENSION / width,
      ANTHROPIC_MANY_IMAGE_MAX_DIMENSION / height,
    );
    const targetWidth = Math.max(
      1,
      Math.min(ANTHROPIC_MANY_IMAGE_MAX_DIMENSION, Math.round(width * scale)),
    );
    const targetHeight = Math.max(
      1,
      Math.min(ANTHROPIC_MANY_IMAGE_MAX_DIMENSION, Math.round(height * scale)),
    );
    ...
    logger.warn(
      'anthropic: failed to resize oversized image for many-image request',
      {
...
async function resizeAnthropicManyImageMessage(
  ...
  if (message.role === 'user' || message.role === 'developer') {
    ...
    const content = await resizeAnthropicManyImageContent(
      message.content,
      state,
      limit,
    );
...
function convertContentBlocks(
  ...
  for (const block of content) {
    ...
    if (!mediaType) {
      blocks.push({
        type: 'text',
        text: `[unsupported image: ${block.mimeType}]`,
      });
      continue;
    }
...
function resolveAnthropicBaseUrl(
  ...
  if (model.provider === 'github-copilot') {
    return normalizeAnthropicBaseUrl(
      resolveGitHubCopilotBaseUrl(model.baseUrl, apiKey) ?? model.baseUrl,
    );
  }
...
export function resolveAnthropicCustomHeadersForBaseUrl(
  ...
  if (!isFoundryEnabled() && isOfficialAnthropicApiUrl(baseUrl))
    return undefined;
...
function resolveFoundryTlsOptions(
  ...
  if (foundryTlsOptionsCache.has(cacheKey))
    return foundryTlsOptionsCache.get(cacheKey);
...
function createAnthropicSseStreamError(data: string): Error {
  ...
  return new AIError.ProviderResponseError(data, {
    provider: 'anthropic',
    kind: 'output',
  });
}
    ...
    try {
      ...
      if (event.type !== sse.event) {
        reportAnthropicEnvelopeAnomaly(
          `event type ${event.type} does not match SSE event ${sse.event}`,
        );
      }
...
function hasAnthropicRawResponseRequest(
  request: unknown,
): request is AnthropicRawResponseRequest {
...
function hasAnthropicStreamWithResponseRequest(
  request: unknown,
): request is AnthropicStreamWithResponseRequest {
 ...
 */
export function isProviderRetryableError(
  error: unknown,
  provider?: string,
): boolean {
...
function unwrapAnthropicThinkingEnvelope(text: string): string | undefined {
  ...
  let stripped = false;
  while (
    current.startsWith(THINKING_ENVELOPE_OPEN) &&
    current.endsWith(THINKING_ENVELOPE_CLOSE)
  ) {
    current = current
      .slice(
        THINKING_ENVELOPE_OPEN.length,
        current.length - THINKING_ENVELOPE_CLOSE.length,
      )
      .trim();
    stripped = true;
  }
 ...
 */
export function applyAnthropicUsageExtras(
  usage: Usage,
  source: AnthropicUsageLike,
): void {
...
function parseAnthropicFallbackWireBlock(
  value: unknown,
): AnthropicFallbackContent | undefined {
  if (!isRecord(value) || value.type !== 'fallback') return undefined;
  const from =
    isRecord(value.from) && typeof value.from.model === 'string'
      ? value.from.model
      : undefined;
  const to =
    isRecord(value.to) && typeof value.to.model === 'string'
      ? value.to.model
      : undefined;
 ...
 */
function fallbackServedModelFromUsage(
  source: AnthropicWireUsage,
): string | undefined {
  const iterations = source.iterations ?? [];
  for (let index = iterations.length - 1; index >= 0; index -= 1) {
    const iteration = iterations[index];
    if (iteration?.type === 'fallback_message' && iteration.model?.trim())
...
function calculateFallbackTurnCost(
  ...
  const hasFallbackMessage = iterations.some(
  ...
  for (const iteration of iterations) {
    ...
    iterationUsage.totalTokens =
      iterationUsage.input +
      iterationUsage.output +
      iterationUsage.cacheRead +
      iterationUsage.cacheWrite;
    calculateCost(
      resolveIterationModel(requestModel, iteration.model),
      iterationUsage,
    );
 ...
 */
export function maybeAddReplayUnsignedThinkingHint(
  model: Model<'anthropic-messages'>,
  message: string,
): string {
...
const streamAnthropicOnce = (
  ...
  (async () => {
    ...
    const rawSseObserver = onSseEvent
      ? (event: RawSseEvent) => onSseEvent(event, model)
      : undefined;
    ...
    try {
      ...
      const supportsEagerToolInputStreaming =
      ...
      let forceDemoteUnsignedThinking =
        providerSessionState?.replayUnsignedThinkingDisabled ?? false;
      const mergedCallerHeaders = mergeHeaders(model.headers, options?.headers);
      const umansGatewayWebSearchHeader = getUmansWebSearchHeader(
        model,
        mergedCallerHeaders,
      );
      ...
      if (
        ...
        fallbacks = fallbacks.map((entry) => {
          ...
          return {
            ...entry,
            output_config:
              outputConfig.task_budget === undefined
                ? undefined
                : { task_budget: outputConfig.task_budget },
          };
        });
      }
      ...
      if (options?.client) {
        ...
        if (
          model.reasoning &&
          model.provider !== 'google-vertex' &&
          ((options?.thinkingEnabled && options.effort !== 'adaptive') ||
        ...
        if (
          ...
          getCacheControl(model, options?.cacheRetention, false).cacheControl
            ?.ttl === '1h' &&
        ...
        if (fallbacks?.length) {
          ...
          for (const entry of fallbacks) {
            ...
            if (
              entry.output_config?.effort &&
              !extraBetas.includes(effortBeta)
            ) {
              extraBetas.push(effortBeta);
            }
            if (
              entry.output_config?.task_budget &&
              !extraBetas.includes(taskBudgetBeta)
            ) {
              extraBetas.push(taskBudgetBeta);
            }
          }
      ...
      const preparedContext = await prepareAnthropicManyImageContext(
        context,
      ...
      const finalizeStreamBlock = (
        ...
        if (block.type === 'text') {
          ...
          const unwrappedThinking = unwrapAnthropicThinkingEnvelope(
            block.thinking,
          );
          ...
          if (partialJson) {
            try {
              ...
              if (isRecord(input)) {
                ...
                reportAnthropicEnvelopeAnomaly(
                  'server_tool_use input is not a JSON object',
                );
              }
            } catch (parseError) {
              reportAnthropicEnvelopeAnomaly(
          ...
          try {
            block.arguments = parseJsonWithRepair(
              finalJson,
      ...
      while (true) {
        ...
        const injectedClientEffortHeaders =
          ...
          (params.output_config as AnthropicOutputConfig | undefined)
            ?.effort !== undefined
        ...
        try {
          ...
          const timedAnthropicStream = iterateWithIdleTimeout(anthropicStream, {
            ...
            onIdle: () =>
              activeAbortTracker.abortLocally(idleTimeoutAbortError),
            onFirstItemTimeout: () =>
              activeAbortTracker.abortLocally(firstEventTimeoutAbortError),
          ...
          for await (const event of observedAnthropicStream) {
            ...
            if (event.type === 'message_start') {
              ...
              if (startUsage) {
                ...
                if (serverSideFallback) {
                  ...
                  if (
                    !calculateFallbackTurnCost(model, output.usage, startUsage)
                  ) {
            ...
            if (event.type === 'content_block_start') {
              if (sawTerminalEnvelope) {
                reportAnthropicEnvelopeAnomaly(
                  `received ${event.type} after terminal stop signal`,
                );
                continue;
              }
              if (openBlocks.has(event.index)) {
                reportAnthropicEnvelopeAnomaly(
                  `duplicate content_block_start index ${event.index}`,
                );
                continue;
              }
              ...
              if (!event.content_block?.type) {
                reportAnthropicEnvelopeAnomaly(
                  'content_block_start missing content_block payload',
                );
                continue;
              }
              ...
              if (event.content_block.type === 'fallback') {
                ...
                const fallback = parseAnthropicFallbackWireBlock(
                  event.content_block,
                );
                if (!serverSideFallback || !fallback) {
                  if (!fallback) {
                    reportAnthropicEnvelopeAnomaly(
                      'fallback content_block missing model refs',
                    );
                  }
                  openBlocks.set(event.index, {
                    contentIndex: -1,
              ...
              if (!event.delta?.type) {
                reportAnthropicEnvelopeAnomaly(
                  'content_block_delta missing delta payload',
                );
                continue;
              }
              ...
              if (event.delta.type === 'text_delta') {
                if (openBlock.kind !== 'text' || block?.type !== 'text') {
                  reportAnthropicEnvelopeAnomaly(
                    `received text_delta for ${openBlock.kind} block`,
                  );
                  continue;
                }
                streamedReplayUnsafeContent = true;
              ...
              if (sawTerminalEnvelope) {
                ...
                reportAnthropicEnvelopeAnomaly(
                  'received message_delta after terminal stop signal',
                );
                continue;
              }
              ...
              if (output.stopReason === 'error') {
                ...
                output.stopDetails =
                  stopDetails ??
                  (rawStopReason ? { type: rawStopReason } : null);
                if (stopDetails?.type === 'refusal') {
                  ...
                  output.errorMessage = explanation
                    ? `${label}: ${explanation}`
                    : label;
              ...
              if (deltaUsage) {
                ...
                if (serverSideFallback) {
                  ...
                  if (served) output.model = served;
                  if (
                    !calculateFallbackTurnCost(model, output.usage, deltaUsage)
                  ) {
                    calculateCost(model, output.usage);
                  }
                } else {
          ...
          const firstEventTimeoutError =
            activeAbortTracker.getLocalAbortReason();
          ...
          if (openBlocks.size > 0) {
            for (const [openIndex, openBlock] of openBlocks) {
              ...
              if (openBlock.kind === 'ignored' || openBlock.contentIndex < 0)
                continue;
              const danglingBlock = blocks[openBlock.contentIndex];
              if (danglingBlock)
                finalizeStreamBlock(danglingBlock, openBlock.contentIndex);
            }
            openBlocks.clear();
          }
          ...
          const streamFailure =
            activeAbortTracker.getLocalAbortReason() ?? streamError;
          if (
            !disableStrictTools &&
            ...
            logger.warn(
              'anthropic: strict tools rejected, retrying without strict tools',
              {
                model: model.id,
            ...
            output.usage = createEmptyUsage(
              copilotDynamicHeaders?.premiumRequests,
            );
          ...
          if (
            ...
            logger.debug(
              'anthropic: fast mode unsupported, retrying without speed',
              {
                model: model.id,
                error:
                  streamFailure instanceof Error
          ...
          const isTransientEnvelopeFailure =
            AIError.isTransientStreamParseError(streamFailure) ||
            AIError.isStreamEnvelopeError(streamFailure);
          const isLocalIdleTimeout =
            streamFailure === idleTimeoutAbortError ||
            (streamFailure instanceof Error &&
              streamFailure.message === idleTimeoutAbortError.message);
          ...
          const backoffDelayMs = calculateAnthropicRetryDelayMs(
          ...
          const headerDelayMs = getRetryAfterMsFromHeaders(
          ...
          if (
            ...
            maxRetryDelayMs > 0 &&
            headerDelayMs > maxRetryDelayMs
          ) {
            throw streamFailure;
          }
          const delayMs =
            headerDelayMs !== undefined
              ? Math.max(headerDelayMs, backoffDelayMs)
              : backoffDelayMs;
      ...
      output.errorMessage = maybeAddReplayUnsignedThinkingHint(
        model,
        result.message,
      );
      output.duration = performance.now() - startTime;
...
export function buildAnthropicSystemBlocks(
  ...
  const trimmedInstructions = extraInstructions
    .map((instruction) => instruction.trim())
    .filter(Boolean);
  const hasBillingHeader = sanitizedPrompts.some((prompt) =>
    prompt.startsWith(CLAUDE_BILLING_HEADER_PREFIX),
  );
  ...
  if (includeClaudeCodeInstruction && !hasBillingHeader) {
    const blocks: AnthropicSystemBlock[] = [
      {
        type: 'text',
        text: createClaudeBillingHeader(firstUserMessageText ?? ''),
      },
  ...
  if (
    cacheControl &&
    lastIndex >= 0 &&
    blocks[lastIndex].cache_control == null
  ) {
    blocks[lastIndex] = {
      ...blocks[lastIndex],
      cache_control: cloneAnthropicCacheControl(cacheControl),
...
export function buildAnthropicClientOptions(
  ...
  const disableStrictTools =
    disableStrictToolsOverride ?? compat.disableStrictTools;
  ...
  const supportsEagerToolInputStreaming = resolveEagerToolInputStreamingSupport(
    model,
    baseUrl,
  );
  ...
  const fetchOptions: AnthropicFetchOptions = {
    ...(tlsFetchOptions ?? {}),
    timeout: false,
  };
  ...
  if (model.provider === 'github-copilot') {
    ...
    // silently produces `Authorization: Bearer ` (no token), which GitHub
    // rejects as "Authorization header is badly formatted" instead of the
    // actual problem (no stored Copilot credential).
    if (!apiKey) {
      throw new AIError.MissingApiKeyError(
        model.provider,
        'No GitHub Copilot credential available. Sign in again via /login github-copilot.',
      );
    }
    ...
    const defaultHeaders = mergeHeaders(
      {
        ...
        ...(betaFeatures.length > 0
          ? { 'anthropic-beta': buildBetaHeader([], betaFeatures) }
          : {}),
      },
      model.headers,
      dynamicHeaders,
  ...
  // asked for one.
  const authorizationHeader = getHeaderCaseInsensitive(
    defaultHeaders,
    'Authorization',
  );
...
function createClient(
  ...
  const { isOAuthToken: oauthToken, ...clientOptions } =
...
function applyPromptCaching(
  ...
  if (
    ...
    isCCLayout =
      ...
      (params.system[0] as { text?: string }).text?.startsWith(
...
function normalizeCacheControlTtlOrdering(
  params: MessageCreateParamsStreaming,
): void {
  const seenFiveMinute = { value: false };
  if (params.tools) {
    for (const tool of params.tools as Array<
      AnthropicWireTool & CacheControlBlock
    >) {
      normalizeCacheControlBlockTtl(tool, seenFiveMinute);
    }
  }
  if (params.system && Array.isArray(params.system)) {
    for (const block of params.system as Array<
      AnthropicSystemBlock & CacheControlBlock
    >) {
  ...
  for (const message of params.messages) {
    ...
    for (const block of message.content as Array<
      ContentBlockParam & CacheControlBlock
    >) {
      normalizeCacheControlBlockTtl(block, seenFiveMinute);
    }
  }
...
function stripCacheControlExceptIndex<T extends CacheControlBlock>(
  ...
  for (
    let index = 0;
    index < blocks.length && excessCounter.value > 0;
    index++
  ) {
...
function countCacheControlBreakpoints(
  params: MessageCreateParamsStreaming,
): number {
  let total = 0;
  if (params.tools) {
...
function enforceCacheControlLimit(
  ...
  const systemBlocks =
    params.system && Array.isArray(params.system)
      ...
      : [];
  const toolBlocks = (params.tools ?? []) as Array<
    AnthropicWireTool & CacheControlBlock
  >;
...
function usesAdaptiveThinkingTagOnly(
  model: Model<'anthropic-messages'>,
): boolean {
...
function resolveAnthropicAdaptiveEffort(
  ...
  if (options.effort)
    return usesAdaptiveThinkingTagOnly(model) ? 'adaptive' : options.effort;
  const requestedEffort = options.reasoning;
  if (!requestedEffort) return undefined;
  return mapEffortToAnthropicAdaptiveEffort(model, requestedEffort);
}

function extractClaudeCodeFirstUserMessageText(
...
function buildParams(
  ...
  const shouldInjectClaudeCodeInstruction =
    isOAuthToken && !model.id.startsWith('claude-3-5-haiku');
  ...
  if (outputConfigEffort && model.provider !== 'google-vertex')
    outputConfigEntries.effort = outputConfigEffort;
  if (options?.taskBudget) outputConfigEntries.task_budget = options.taskBudget;
  const outputConfig = Object.keys(outputConfigEntries).length
    ? outputConfigEntries
    : undefined;
  ...
  const maxOutputTokens = isOAuthToken
  ...
  const allowSamplingParams =
    model.compat.supportsSamplingParams &&
    (thinkingType === undefined || thinkingType === 'disabled');
  ...
  if (options?.stopSequences?.length) {
    ...
    if (
      seqs.length > ANTHROPIC_STOP_SEQUENCES_MAX &&
      !warnedStopSequencesTrim
    ) {
      warnedStopSequencesTrim = true;
      logger.warn(
        'anthropic: stop_sequences exceeds 4; extra entries dropped',
        {
    ...
    params.stop_sequences =
      seqs.length > ANTHROPIC_STOP_SEQUENCES_MAX
        ? seqs.slice(0, ANTHROPIC_STOP_SEQUENCES_MAX)
        : seqs;
  }
...
function isEmptyToolResultWireContent(
...
function buildToolResultBlock(
  ...
  let content = convertContentBlocks(
    msg.content,
    model.input.includes('image'),
  );
...
export function convertAnthropicMessages(
  ...
  for (let i = 0; i < transformedMessages.length; i++) {
    ...
    if (msg.role === 'user' || msg.role === 'developer') {
      ...
      if (typeof msg.content === 'string') {
        ...
        const contentBlocks = convertContentBlocks(
          msg.content,
          model.input.includes('image'),
        );
      ...
      for (const block of msg.content) {
        if (block.type === 'text') {
          ...
          if (
            !opts?.serverSideFallbackEnabled ||
            !model.compat.officialEndpoint
          )
            continue;
          blocks.push({
  ...
  if (
    ...
    for (const idx of developerParamIndices) {
      ...
      const lastOrBeforeAssistant =
        idx === params.length - 1 || next?.role === 'assistant';
...
const ANTHROPIC_TOOL_SCHEMA_OBJECT_KEEP = new Set([
  'properties',
  'required',
  'additionalProperties',
]);
/** Keys preserved on `type: "array"` nodes; `minItems` only when its value is 0 or 1. */
const ANTHROPIC_TOOL_SCHEMA_ARRAY_KEEP = new Set([
  'items',
  'prefixItems',
  'minItems',
]);
...
const ANTHROPIC_STRICT_TOOL_ALLOWLIST = new Set([
  'bash',
  'python',
  'edit',
  'find',
...
function isJsonSchemaArrayNode(schema: Record<string, unknown>): boolean {
  ...
  if (Array.isArray(t) && t.includes('array') && !t.includes('object'))
    return true;
...
function pickAnthropicEffectiveScalarType(
...
function normalizeAnthropicToolSchemaNode(
  ...
  if (Array.isArray(schema))
    return schema.map((entry) =>
      normalizeAnthropicToolSchemaNode(entry, cache),
    );
  ...
  for (const key in schema) {
    ...
    const isRootCombinator =
      isRoot &&
      COMBINATOR_KEYS.includes(key as (typeof COMBINATOR_KEYS)[number]);
    if (
      !isRootCombinator &&
      (ANTHROPIC_TOOL_SCHEMA_UNIVERSAL_KEEP.has(key) || perTypeKeep?.has(key))
    ) {
      result[key] = value;
    } else {
  ...
  if (scalarType === 'string') {
    ...
    if (
      ...
      !ANTHROPIC_TOOL_SCHEMA_STRING_FORMATS.has(format)
    ) {
  ...
  if (isRecord(result.properties)) {
    ...
    for (const propName in sourceProperties) {
      ...
      normalizedProperties[propName] = normalizeAnthropicToolSchemaNode(
        sourceProperties[propName],
        cache,
      );
    }
  ...
  if (isRecord(result.additionalProperties)) {
    const normalized = normalizeAnthropicToolSchemaNode(
  ...
  if (Array.isArray(result.items)) {
    result.items = result.items.map((item) =>
      normalizeAnthropicToolSchemaNode(item, cache),
    );
  ...
  for (const key of COMBINATOR_KEYS) {
    ...
    if (Array.isArray(variants)) {
      result[key] = variants.map((variant) =>
        normalizeAnthropicToolSchemaNode(variant, cache),
      );
    }
  }
  for (const defsKey of ['$defs', 'definitions'] as const) {
    ...
    for (const name in sourceDefs) {
      ...
      normalizedDefs[name] = normalizeAnthropicToolSchemaNode(
        sourceDefs[name],
        cache,
      );
    }
...
function hasNullVariant(schema: Record<string, unknown>): boolean {
  ...
  return (
    ...
    schema.anyOf.some((variant) => isRecord(variant) && variant.type === 'null')
  );
}
function hasAnthropicSchemaDefiningKeyword(
  schema: Record<string, unknown>,
): boolean {
  if (
...
function makeAnthropicNullableSchema(
  schema: unknown,
...
function normalizeAnthropicStrictSchemaNode(
  ...
  if (Array.isArray(schema)) {
    ...
    for (const entry of schema) {
      const normalized = normalizeAnthropicStrictSchemaNode(
        entry,
        budget,
        cache,
      );
  ...
  if (isRecord(result.properties)) {
    ...
    for (const [propertyName, propertySchema] of Object.entries(
      ...
      const normalizedProperty = normalizeAnthropicStrictSchemaNode(
        propertySchema,
        budget,
        cache,
      );
      ...
      const nullableProperty = makeAnthropicNullableSchema(
        normalizedProperty,
        budget,
      );
  ...
  if (Array.isArray(result.items)) {
    const items = normalizeAnthropicStrictSchemaNode(
      result.items,
      budget,
      cache,
    );
  ...
  if (Array.isArray(result.prefixItems)) {
    const prefixItems = normalizeAnthropicStrictSchemaNode(
      result.prefixItems,
      budget,
      cache,
    );
  ...
  for (const key of COMBINATOR_KEYS) {
    ...
    const normalizedVariants = normalizeAnthropicStrictSchemaNode(
      variants,
      budget,
      cache,
    );
  ...
  for (const defsKey of ['$defs', 'definitions'] as const) {
    ...
    for (const [definitionName, definitionSchema] of Object.entries(
      ...
      const normalizedDefinition = normalizeAnthropicStrictSchemaNode(
        definitionSchema,
        budget,
        cache,
      );
 ...
 */
function hasAnthropicStrictIncompatibleKeyword(
  schema: unknown,
  seen = new Set<object>(),
): boolean {
  if (Array.isArray(schema)) {
    ...
    return schema.some((entry) =>
      hasAnthropicStrictIncompatibleKeyword(entry, seen),
    );
  }
  ...
  return Object.values(schema).some((value) =>
    hasAnthropicStrictIncompatibleKeyword(value, seen),
  );
}
...
function buildAnthropicBaseToolInputSchema(
  tool: Tool,
...
function buildAnthropicToolSchemaPlans(
  tools: Tool[],
  disableStrictTools = false,
): AnthropicToolSchemaPlan[] {
  const plans = tools.map(
    (tool): AnthropicToolSchemaPlan => ({
      inputSchema: buildAnthropicBaseToolInputSchema(
        tool,
      ) as AnthropicToolInputSchema,
      strict: false,
    }),
  );
