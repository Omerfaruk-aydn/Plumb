 ...
 *
 * PLUMB runtime adaptation: installs a minimal `Bun` global for PLUMB's
 * provider modules when running on Node. The ambient `bun-types.d.ts`
 * already supplies the compile-time declarations; this module provides the
 * corresponding runtime values.
 *
 * Surface implemented (from the actual usage scan of the provider modules):
