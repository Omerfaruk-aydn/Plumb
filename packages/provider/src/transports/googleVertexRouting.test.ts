 ...
 *
 * Production-shaped regression: every google-vertex catalog model dialect
 * (native Gemini `google-vertex`, Claude-on-Vertex `anthropic-messages`,
 * Llama/GPT-OSS MaaS `openai-completions`) must reach a real, resolvable
 * URL with {project}/{location} substituted and a real Google OAuth Bearer
 * token -- never the literal `{location}-aiplatform.googleapis.com`
 * hostname, and never the direct-API `x-api-key`/`?key=` auth schemes.
 */
