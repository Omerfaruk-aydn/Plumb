...
import { PREVIEW_GEMINI_FLASH_MODEL, PLUMB_DIR } from '@plumb/core';
export { PLUMB_DIR };
...
export class TestRig {
  ...
  private _createSettingsFile(overrideSettings?: Record<string, unknown>) {
    const projectGeminiDir = join(this.testDir!, PLUMB_DIR);
    mkdirSync(projectGeminiDir, { recursive: true });

    const userGeminiDir = join(this.homeDir!, PLUMB_DIR);
  ...
  /**
   * The command and args to use to invoke PLUMB. Allows us to switch
  ...
  addTestMcpServer(name: string, config: TestMcpConfig | string) {
    ...
    const updateSettings = (dir: string) => {
      const settingsPath = join(dir, PLUMB_DIR, 'settings.json');
      let settings: any = {};
      if (fs.existsSync(settingsPath)) {
        settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
      } else {
        fs.mkdirSync(join(dir, PLUMB_DIR), { recursive: true });
      }
