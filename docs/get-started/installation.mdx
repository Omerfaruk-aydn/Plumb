...
# PLUMB installation, execution, and releases

This document provides an overview of PLUMB's system requirements,
...
We recommend most users install PLUMB using one of the following
installation methods. Note that PLUMB comes pre-installed on
...
<Tabs>
  ...
  # Install PLUMB globally via npm (inside the environment)
  npm install -g plumb-cli
  ```
...
For most users, we recommend running PLUMB with the `gemini` command:
...
You can also run PLUMB using one of the following advanced methods:

<Tabs>
  <TabItem label="npx">

  Run instantly with npx. You can run PLUMB without permanent installation.
  ...
  For security and isolation, PLUMB can be run inside a container. This is
  ...
  - **Directly from the registry:** You can run the published sandbox image
    ...
    ```
  - **Using the `--sandbox` flag:** If you have PLUMB installed locally
...
PLUMB has three release channels: stable, preview, and nightly. For most
