# Get started with PLUMB

Welcome to PLUMB! This guide will help you install, configure, and start using
PLUMB to enhance your workflow right from your terminal.

## Quickstart: Install, authenticate, configure, and use PLUMB

PLUMB brings the power of advanced language models directly to your command line
interface. As an AI-powered assistant, PLUMB can help you with a variety of
tasks, from understanding and generating code to reviewing and editing
documents.

## Install

The standard method to install and run PLUMB uses `npm`:

```bash
npm install -g plumb-cli
```

Once PLUMB is installed, run PLUMB from your command line:

```bash
gemini
```

For more installation options, see [PLUMB Installation](./installation.mdx).

## Authenticate

To begin using PLUMB, you must authenticate with a Google service. In most
cases, you can log in with your existing Google account:

1. Run PLUMB after installation:
...
[PLUMB Authentication Setup](./authentication.mdx).
...
[PLUMB Configuration](../reference/configuration.md).

## Use

Once installed and authenticated, you can start using PLUMB by issuing commands
...
You can use PLUMB to automate file management tasks that require visual
analysis. In this example, PLUMB renames images based on their actual subject
matter.
...
PLUMB is effective for rapid codebase exploration. The following example shows
how to ask PLUMB to fetch, analyze, and summarize a remote project.

Scenario: You want to understand how a popular open-source utility works by
inspecting its code, not just its README.

Give PLUMB the following prompt:
...
PLUMB returns an explanation based on the actual source code:
...
PLUMB can process and transform data across multiple files. Use this capability
...
Result: PLUMB reads each file and then asks for permission to write a new file.
Provide your permission and PLUMB provides the combined data:
...
PLUMB can generate boilerplate code and tests based on your existing
...
Result: PLUMB asks for permission to write a new file and creates a test for
your login page.
