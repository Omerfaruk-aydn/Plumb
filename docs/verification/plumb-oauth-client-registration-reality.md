...
All OAuth client registrations in the current codebase use client IDs from
other products' CLI tools. These are not publicly reusable registrations.
...
| Google Gemini CLI | Standard Google OAuth | YES (if registered) | IMPLEMENTATION_COMPLETE_EXTERNAL_LOGIN_REQUIRED |
...
- OAuth providers are displayed in the UI with "Press Enter to sign in with your browser"
- The browser opens the provider's OAuth page
- The callback server listens on the configured port
- If the client ID is rejected by the provider, the user sees a provider-side error
