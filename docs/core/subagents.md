...
Subagents are specialized agents that operate within your main PLUMB session.
...
PLUMB's main agent is instructed to use specialized subagents when a task
...
PLUMB comes with the following built-in subagents:
...
- **Name:** `cli_help`
- **Purpose:** Get expert knowledge about PLUMB itself, its commands,
...
server is bundled with PLUMB and launched automatically — no separate
...
The first time the browser agent is invoked, PLUMB displays a consent dialog.
...
Subagent tool isolation moves PLUMB away from a single global tool registry. By
providing isolated execution environments, you can ensure that subagents only
...
PLUMB can also delegate tasks to remote subagents using the Agent-to-Agent (A2A)
protocol.
