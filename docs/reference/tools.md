...
PLUMB uses tools to interact with your local environment, access information,
and perform actions on your behalf. These tools extend the model's capabilities
beyond text generation, letting it read files, execute commands, and search the
web.

## How to use PLUMB's tools

Tools are generally invoked automatically by PLUMB when it needs to perform an
action. However, you can also trigger specific tools manually using shorthand
syntax.

### Automatic execution and security

When the model wants to use a tool, PLUMB evaluates the request against its
security policies.
...
| [`get_internal_docs`](../tools/internal-docs.md) | `Think` | Accesses PLUMB's own documentation for accurate answers about its capabilities. |
...
You can extend PLUMB with custom tools by configuring `tools.discoveryCommand`
in your settings or by connecting to MCP servers.
