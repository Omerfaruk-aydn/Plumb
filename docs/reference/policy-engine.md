...
PLUMB includes a powerful policy engine that provides fine-grained control over
tool execution. It allows users and administrators to define rules that
...
2.  **Create a new policy file** (for example,
    ...
    priority = 100
    ```
3.  **Run a command** that triggers the policy (for example, ask PLUMB to
...
| :-------- | :--- | :-------------------------------------------------------------------------------------------- |
| Default   | 1    | Built-in policies that ship with PLUMB.                                                       |
...
PLUMB ships with a set of default policies to provide a safe out-of-the-box
experience.
