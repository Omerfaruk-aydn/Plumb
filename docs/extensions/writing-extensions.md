# Build PLUMB extensions

PLUMB extensions let you expand the capabilities of PLUMB by adding custom
tools, commands, and context. This guide walks you through creating your first
extension, from setting up a template to adding custom functionality and linking
it for local development.

## Prerequisites

Before you start, ensure you have PLUMB installed and a basic understanding of
Node.js.

## Extension features

Extensions offer several ways to customize PLUMB. Use this table to decide which
...
The manifest file tells PLUMB how to load and use your extension.
...
When a user installs this extension, PLUMB will prompt them to enter the "API
Key". The value will be stored securely in the system keychain (because
...
Link your extension to your PLUMB installation for local development.
...
2.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from PLUMB extensions directory
    to your development directory. Changes you make are reflected immediately.

    ```bash
    gemini extensions link .
    ```

Restart your PLUMB session to use the new `fetch_posts` tool. Test it by asking:
"fetch posts".
...
After saving the file, restart PLUMB. Run `/fs:grep-code "some pattern"` to use
your new command.

## Step 6: Add a custom `PLUMB.md`

Provide persistent context to the model by adding a `PLUMB.md` file to your
extension. This is useful for setting behavior or providing essential tool
information.

1.  Create a file named `PLUMB.md` in the root of your extension directory:
...
Restart PLUMB. The model now has the context from your `PLUMB.md` file in every
...
PLUMB automatically discovers skills bundled with your extension. The model
