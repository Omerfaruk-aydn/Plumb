...
Hooks are scripts or programs that PLUMB executes at specific points in the
agentic loop, allowing you to intercept and customize behavior without modifying
the CLI's source code.

## What are hooks?

Hooks run synchronously as part of the agent loop—when a hook event fires, PLUMB
...
Hooks are triggered by specific events in PLUMB's lifecycle.
...
3. **Debug via Stderr**: Use `stderr` for **all** logging and debugging (for
   example, `echo "debug" >&2`). PLUMB captures `stderr` but never attempts to
   parse it as JSON.

#### Exit codes

PLUMB uses exit codes to determine the high-level outcome of a hook execution:
...
Hooks are configured in `settings.json`. PLUMB merges configurations from
...
PLUMB **fingerprints** project hooks. If a hook's name or command changes (for
example, via `git pull`), it is treated as a **new, untrusted hook** and you
will be warned before it executes.
