...
PLUMB uses exit codes for high-level flow control:
...
1. **Detection**: PLUMB detects the hooks.
...
> changed (for example, by a `git pull`), its identity changes. PLUMB will treat
...
Hooks inherit the environment of PLUMB process, which may include sensitive API
keys. PLUMB provides a
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. PLUMB defaults
