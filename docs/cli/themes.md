...
PLUMB supports a variety of themes to customize its color scheme and appearance.
You can change the theme to suit your preferences via the `/theme` command or
`"theme":` configuration setting.

## Available themes

PLUMB comes with a selection of pre-defined themes, which you can list using the
`/theme` command within PLUMB:
...
Selected themes are saved in PLUMB's
...
PLUMB lets you create your own custom color themes by specifying them in your
...
<!-- prettier-ignore -->
> [!WARNING]
> For your safety, PLUMB will only load theme files that
...
- Select your custom theme using the `/theme` command in PLUMB. Your custom
