...
Model steering lets you provide real-time guidance and feedback to PLUMB while
it is actively executing a task. This lets you correct course, add missing
...
When model steering is enabled, PLUMB treats any text you type while the agent
is working as a steering hint.
...
PLUMB acknowledges your hint with a brief message and injects it directly into
the model's context for the very next turn. The model then re-evaluates its
...
When you submit a steering hint, PLUMB performs the following actions:
