...
prompts as personal shortcuts within PLUMB. You can create commands that are
...
PLUMB discovers commands from two locations, loaded in a specific order:
...
When a custom command attempts to execute a shell command, PLUMB will now prompt
...
PLUMB will then execute the multi-line prompt defined in your TOML file.
