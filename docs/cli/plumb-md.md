# Provide context with PLUMB.md files

Context files, which use the default name `PLUMB.md`, are a powerful feature for
...
1.  **Global context file:**
    ...
    - **Location:** `~/.gemini/PLUMB.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**

    - **Location:** The CLI searches for `PLUMB.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `PLUMB.md` files in that directory and its
...
### Example `PLUMB.md` file

Here is an example of what you can include in a `PLUMB.md` file at the root of a
TypeScript project:
...
- **`/memory reload`**: Forces a re-scan and reload of all `PLUMB.md` files from
  all configured locations.

## Modularize context with imports

You can break down large `PLUMB.md` files into smaller, more manageable
...
While `PLUMB.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "PLUMB.md"]
  }
}
```
