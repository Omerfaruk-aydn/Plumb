...
PLUMB provides built-in support for OpenTelemetry, transforming every agent
...
enterprise-wide usage in the cloud, PLUMB's observability system provides the
actionable intelligence needed to move from "black box" AI to predictable,
high-performance systems.

## OpenTelemetry integration

PLUMB integrates with **[OpenTelemetry]**, a vendor-neutral, industry-standard
...
2.  Authenticate with Google Cloud using one of these methods:
    ...
    * **Method B: CLI Auth** (Direct export only): Simplest method for local
      users. PLUMB uses the same OAuth credentials you used for login. To enable
      this, set `useCliAuth: true` in your `.gemini/settings.json`:
...
After you enable telemetry and run PLUMB, you can view your data in the Google
Cloud Console.
...
PLUMB provides a pre-configured
...
![PLUMB Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![PLUMB Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)

![PLUMB Monitoring Dashboard Logs](/docs/assets/monitoring-dashboard-logs.png)

To learn more, see
[Instant insights: PLUMB’s pre-configured monitoring dashboards](https://cloud.google.com/blog/topics/developers-practitioners/instant-insights-gemini-clis-new-pre-configured-monitoring-dashboards/).
...
PLUMB includes identifiers in its `User-Agent` header to help you differentiate
and report on API traffic from different environments (for example, identifying
calls from Gemini Code Assist versus a standard terminal).
...
PLUMB includes `session.id`, `installation.id`, `active_approval_mode`, and
`user.email` (when authenticated) as common attributes on all data.

### Logs

Logs provide timestamped records of specific events. PLUMB logs events across
several categories.
...
These logs track how PLUMB selects and routes requests to models.
...
PLUMB exports several custom metrics.
...
PLUMB provides detailed performance metrics for advanced monitoring.
