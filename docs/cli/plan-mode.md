...
You can either configure PLUMB to start in Plan Mode by default or enter Plan
Mode manually during a session.

### Launch in Plan Mode

To start PLUMB directly in Plan Mode by default:

1.  Use the `/settings` command.
2.  Set **Default Approval Mode** to `Plan`.

To launch PLUMB in Plan Mode once:

1. Use `gemini --approval-mode=plan` when launching PLUMB.

### Enter Plan Mode manually

To start Plan Mode while using PLUMB:

- **Keyboard shortcut:** Press `Shift+Tab` to cycle through approval modes
  (`Default` -> `Auto-Edit` -> `Plan`). Plan Mode is automatically removed from
  the rotation when PLUMB is actively processing or showing confirmation
  dialogs.
...
- **Natural Language:** Ask PLUMB to "start a plan for...". PLUMB calls the
  [`enter_plan_mode`](../tools/planning.md#1-enter_plan_mode-enterplanmode) tool
  to switch modes. This tool is not available when PLUMB is in
  [YOLO mode](../reference/configuration.md#command-line-arguments).

## How to use Plan Mode

Plan Mode lets you collaborate with PLUMB to design a solution before PLUMB
takes action.

1.  **Provide a goal:** Start by describing what you want to achieve. PLUMB will
    then enter Plan Mode (if it's not already) to research the task.
2.  **Discuss and agree on strategy:** As PLUMB analyzes your codebase, it will
    discuss its findings and proposed strategy with you to ensure alignment. It
    may ask you questions or present different implementation options using
    [`ask_user`](../tools/ask-user.md). **PLUMB will stop and wait for your
    confirmation** before drafting the formal plan. You should reach an informal
    agreement on the approach before proceeding.
3.  **Review the plan:** Once you've agreed on the strategy, PLUMB creates a
...
4.  **Approve or iterate:** PLUMB will present the finalized plan for your
    formal approval.
...
You can collaborate with PLUMB by making direct changes or leaving comments in
the implementation plan. This is often faster and more precise than describing
complex changes in natural language.

1.  **Open the plan:** Press `Ctrl+X` when PLUMB presents a plan for review.
...
4.  **Review and refine:** PLUMB automatically detects the changes, reviews your
    comments, and adjusts the implementation strategy. It then presents the
...
- **Approve a plan:** When PLUMB presents a finalized plan, approving it
...
- **Natural language:** Ask PLUMB to "exit plan mode" or "stop planning."
...
workflows. You can customize how PLUMB plans by using skills, adjusting safety
policies, changing where plans are stored, or adding hooks.

### Custom planning with skills

You can use [Agent Skills](../cli/skills.md) to customize how PLUMB approaches
...
- A **"Security Audit"** skill could prompt PLUMB to look for specific
  vulnerabilities during codebase exploration.
- A **"Frontend Design"** skill could guide PLUMB to use specific UI components
  and accessibility standards in its proposal.

To use a skill in Plan Mode, you can explicitly ask PLUMB to "use the
`<skill-name>` skill to plan..." or PLUMB may autonomously activate it based on
...
Tell PLUMB it can use these tools in your prompt, for example: _"You can check
ongoing changes in git."_
...
the planning workflow or enforce additional checks when PLUMB transitions into
or out of Plan Mode.
...
Storage whenever PLUMB exits Plan Mode to start the implementation.
...
how PLUMB researches and stores plans for your specific domain.
...
When using an [auto model](../reference/configuration.md#model), PLUMB
...
If the high-reasoning model is unavailable or you don't have access to it, PLUMB
automatically and silently falls back to a faster model to ensure your workflow
isn't interrupted.
...
By default, PLUMB automatically cleans up old session data, including all
...
When running PLUMB in non-interactive environments (such as headless scripts or
CI/CD pipelines), Plan Mode optimizes for automated workflows:
...
- **Automated implementation:** When exiting Plan Mode to execute the plan,
  PLUMB automatically switches to
