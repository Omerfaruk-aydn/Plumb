...
problems encountered while using PLUMB.

## General issues

This section addresses common questions about PLUMB usage, security, and
troubleshooting general errors.

### Why can't I use third-party software like Claude Code, OpenClaw, or OpenCode with PLUMB?
...
your total token usage using the `/stats` command in PLUMB.

## Installation and updates

### How do I check which version of PLUMB I'm currently running?

You can check your current PLUMB version using one of these methods:

- Run `gemini --version` or `gemini -v` from your terminal
- Check the globally installed version using your package manager:
  - npm: `npm list -g plumb-cli`
  - pnpm: `pnpm list -g plumb-cli`
  - yarn: `yarn global list plumb-cli`
  - bun: `bun pm ls -g plumb-cli`
  - homebrew: `brew list --versions gemini-cli`
- Inside an active PLUMB session, use the `/about` command

### How do I update PLUMB to the latest version?

If you installed it globally via `npm`, update it using the command
`npm install -g plumb-cli@latest`. If you compiled it from source, pull the
...
- **Use a `.env` file:** Create a `.env` file in your project's `.gemini`
  directory (`.gemini/.env`) and store your keys there. PLUMB will automatically
...
### Where are PLUMB configuration and settings files stored?

PLUMB configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.gemini/settings.json`.
2.  In your project's root directory: `./.gemini/settings.json`.

Refer to [PLUMB Configuration](../reference/configuration.md) for more details.
...
limits to Gemini Code Assist and PLUMB. These are shared across PLUMB and agent
mode in the IDE. You can confirm you have higher limits by checking if you are
still subscribed to Google AI Pro or Ultra in your
[subscription settings](https://one.google.com).

### What is the privacy policy for using Gemini Code Assist or PLUMB if I've subscribed to Google AI Pro or Ultra?
...
across both Gemini 2.5 Pro and Flash. They are shared quota across PLUMB and
agent mode in Gemini Code Assist IDE extensions. You can learn more about quota
limits for PLUMB, Gemini Code Assist and agent mode in Gemini Code Assist at
[Quotas and limits](https://developers.google.com/gemini-code-assist/resources/quotas).

### If I upgrade to higher limits for PLUMB and Gemini Code Assist by purchasing a Google AI Pro or Ultra subscription, will Gemini start using my data to improve its machine learning models?
...
Search the
[PLUMB Q&A discussions on GitHub](https://github.com/google-gemini/gemini-cli/discussions/categories/q-a)
or
