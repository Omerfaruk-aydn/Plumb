...
ARG SANDBOX_NAME="plumb-sandbox"
ARG CLI_VERSION_ARG
...
RUN npm install -g /tmp/gemini-core.tgz \
  ...
  && node -e "const fs=require('node:fs'); JSON.parse(fs.readFileSync('/usr/local/share/npm-global/lib/node_modules/plumb-cli/package.json','utf8')); JSON.parse(fs.readFileSync('/usr/local/share/npm-global/lib/node_modules/@plumb/core/package.json','utf8'));" \
